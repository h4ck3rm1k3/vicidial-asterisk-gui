...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/astguiclient.php?relogin=YES&user=$user&pass=$pass&phone_login=$phone_login&phone_pass=$phone_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_ru/astguiclient.php?relogin=YES&user=$user&pass=$pass&phone_login=$phone_login&phone_pass=$phone_pass\">Russian <img src=\"../agc/images/ru.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
	...
	if($auth>0)
		...
		if ($WeBRooTWritablE > 0)
			{
			fwrite ($fp, "VICIНАБОР НОМЕРА|GOOD|$date|$user|$pass|$ip|$browser|$LOGfullname|\n");
			fclose($fp);
			}
		}
	else
		...
		if ($WeBRooTWritablE > 0)
			{
			fwrite ($fp, "VICIНАБОР НОМЕРА|FAIL|$date|$user|$pass|$ip|$browser|$LOGfullname|\n");
			fclose($fp);
			}
		}
	}
...
<?	echo "Добро пожаловать $LOGfullname, вы зарегистрированы на телефон: $fullname - $protocol/$extension на $server_ip &nbsp; <a href=\"#\" onclick=\"LogouT();return false;\">ВЫХОД</a><BR>\n"; ?>
</TD></TR>
...
<tr><td align=left><font face="Arial,Helvetica" size=1>Версия web-клиента astGUIclient :<? echo $version ?> СБОРКА:<? echo $build ?></font></td></tr>
</TABLE>
