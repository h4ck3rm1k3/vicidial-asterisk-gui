...
if ($relogin == 'YES')
{
echo "<title>Web client de VICIDIAL: Re-Início de uma sessão</title>\n";
echo "</head>\n";
...
if ($user_login_first == 1)
{
	...
	{
	echo "<title>Web client de VICIDIAL: Início de uma sessão Da Campanha</title>\n";
	echo "</head>\n";
	echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
	echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\">Timeclock</A><BR>\n";
	echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_pt/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Português <img src=\"../agc/images/pt.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
	...
	else
	{
		...
		echo "<title>Web client de VICIDIAL: Início de uma sessão</title>\n";
		echo "</head>\n";
		echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
		echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\">Timeclock</A><BR>\n";
		echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
if ( (strlen($phone_login)<2) or (strlen($phone_pass)<2) )
{
echo "<title>Web client de VICIDIAL:  Início de uma sessão Do Telefone</title>\n";
echo "</head>\n";
echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\">Timeclock</A><BR>\n";
echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
	...
	if($auth>0)
		...
		if ( ( (ereg("COMEÇO|ALL",$shift_enforcement)) and (!ereg("OFF",$VU_agent_shift_enforcement_override)) ) or (ereg("COMEÇO|ALL",$VU_agent_shift_enforcement_override)) )
			...
			if ( ($shift_ok < 1) and ($VU_shift_override_flag < 1) and ($VDloginDISPLAY > 0) )
				...
				$VDdisplayMESSAGE.= "Manager Início de uma sessão: <INPUT TYPE=TEXT NAME=\"MGR_login$loginDATE\" SIZE=10 maxlength=20><br>\n";
		...
		if ( (!eregi(" $VD_campaign ",$LOGallowed_campaigns)) and (!eregi("ALL-CAMPAIGNS",$LOGallowed_campaigns)) )
			{
			echo "<title>Web client de VICIDIAL: VICIDIAL Início de uma sessão Da Campanha</title>\n";
			echo "</head>\n";
			echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
			echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\">Timeclock</A><BR>\n";
			echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
if (!$authphone)
	{
	echo "<title>Web client de VICIDIAL: Início de uma sessão Do Telefone Error</title>\n";
	echo "</head>\n";
	echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
	echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\">Timeclock</A><BR>\n";
	echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
### logged in and use that phone login account
	...
	echo "<title>Web client de VICIDIAL</title>\n";
...
// filter manual dialstring and pass em to originate call
	function SendManualDial(taskFromConf)
		...
		if (taskFromConf == 'YES')
			...
			document.getElementById("DialWithCustomer").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_dialwithcustomer_OFF.gif\" border=0 alt=\"Seletor Com Cliente\"></a>";

			document.getElementById("ParkCustomerDial").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_parkcustomerdial_OFF.gif\" border=0 alt=\"Seletor Do Cliente Do Parque\"></a>";
	...
	function LogouT(tempreason)
		...
		if (MD_channel_look==1)
			...
			if (VD_live_customer_call==1)
				...
				document.getElementById("LogouTBoxLink").innerHTML = logout_content + "<a href=\"" + agcPAGE + "?relogin=YES&session_epoch=" + epoch_sec + "&session_id=" + session_id + "&session_name=" + session_name + "&VD_login=" + user + "&VD_campaign=" + campaign + "&phone_login=" + phone_login + "&phone_pass=" + phone_pass + "&VD_pass=" + pass + "\">ESTALE AQUI PARA LOGON OUTRA VEZ</a>\n";
...
<span style="position:absolute;left:0px;top:0px;z-index:3;" id="Boa vindaBoxA">
    <table border=0 bgcolor="#FFFFFF" width=<?=$CAwidth ?> height=<?=$HKwidth ?>><TR><TD align=center><BR><span id="Boa vindaBoxAt">VICIDIAL</span></TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:5px;top:<?=$HTheight ?>px;z-index:21;" id="TransferMain">
	...
	<font class="body_text">
	 <IMG SRC="../agc/images/vdc_XB_header.gif" border=0 alt="Transferência - Conferência"> &nbsp; &nbsp; &nbsp; &nbsp; <span id="XfeRDiaLGrouPSelecteD"></span> &nbsp; &nbsp; <span id="XfeRCID"></span><BR>
	...
	<span STYLE="background-color: #CCCCCC" id="Leave3WayCall"><a href="#" onclick="leave_3way_call('FIRST');return false;"><IMG SRC="../agc/images/vdc_XB_leave3waycall.gif" border=0 alt="CHAMADA DA LICENÇA 3-WAY"></a></span> 
...
<span style="position:absolute;left:0px;top:0px;z-index:31;" id="AgenTDisablEBoX">
    <table border=1 bgcolor="#FFFFFF" width=<?=$CAwidth ?> height=500><TR><TD align=center>Sua sessão foi incapacitada<BR><a href="#" onclick="LogouT('DISABLED');return false;">LOGOUT</a><BR><BR><a href="#" onclick="hideDiv('AgenTDisablEBoX');return false;">Vá Para trás</a>
</TD></TR></TABLE>
</span>

<span style="position:absolute;left:0px;top:0px;z-index:32;" id="SysteMDisablEBoX">
    <table border=1 bgcolor="#FFFFFF" width=<?=$CAwidth ?> height=500><TR><TD align=center>Há um tempo sincronização problema com o seu sistema, por favor informe o seu administrador de sistema<BR><BR><BR><a href="#" onclick="hideDiv('SysteMDisablEBoX');return false;">Vá Para trás</a>
</TD></TR></TABLE>
</span>
...
<table BGCOLOR="<? echo $MAIN_COLOR ?>" id="MaiNfooter" width=<?=$MNwidth ?>><tr height=32><td height=32><font face="Arial,Helvetica" size=1>Versão do correia-cliente de VICIDIAL: <? echo $version ?> &nbsp; &nbsp; CONFIGURAÇÃO: <? echo $build ?> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Usuário: <? echo $server_ip ?>  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;</font><BR>
