...
$SCwidth =  ($MASTERwidth + 230);	# 670 - live call seconden counter
...
if ($relogin == 'YES')
{
echo "<title>VICIDIAL web client: Opnieuw inloggen</title>\n";
echo "</head>\n";
...
if ($user_login_first == 1)
{
	...
	{
	echo "<title>VICIDIAL web client: Campagne Inloggen</title>\n";
	echo "</head>\n";
	echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
	echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\"> Tijdslot</A><BR>\n";
	echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_nl/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Dutch <img src=\"../agc/images/nl.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
	...
	else
	{
		...
		echo "<title>VICIDIAL web client: Inloggen</title>\n";
		echo "</head>\n";
		echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
		echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\"> Tijdslot</A><BR>\n";
		echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
if ( (strlen($phone_login)<2) or (strlen($phone_pass)<2) )
{
echo "<title>VICIDIAL web client:  Telefoon Inloggen</title>\n";
echo "</head>\n";
echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\"> Tijdslot</A><BR>\n";
echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
	...
	if($auth>0)
		...
		if ( ( (ereg("START|ALL",$shift_enforcement)) and (!ereg("OFF",$VU_agent_shift_enforcement_override)) ) or (ereg("START|ALL",$VU_agent_shift_enforcement_override)) )
			...
			if ( ($shift_ok < 1) and ($VU_shift_override_flag < 1) and ($VDloginDISPLAY > 0) )
				...
				$VDdisplayMESSAGE.= "Manager Inloggen: <INPUT TYPE=TEXT NAME=\"MGR_login$loginDATE\" SIZE=10 maxlength=20><br>\n";
				$VDdisplayMESSAGE.= "Manager Wachtwoord: <INPUT TYPE=PASSWORD NAME=\"MGR_pass$loginDATE\" SIZE=10 maxlength=20><br>\n";
		...
		if ( (!eregi(" $VD_campaign ",$LOGallowed_campaigns)) and (!eregi("ALL-CAMPAIGNS",$LOGallowed_campaigns)) )
			{
			echo "<title>VICIDIAL web client: VICIDIAL Campagne Inloggen</title>\n";
			echo "</head>\n";
			echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
			echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\"> Tijdslot</A><BR>\n";
			echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_nl/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Dutch <img src=\"../agc/images/nl.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";			echo "</TR></TABLE>\n";
		...
		if($CAMPactive>0)
			...
			if ( (!ereg('DISABLED',$VU_vicidial_recording_override)) and ($VU_vicidial_recording > 0) )
				...
				print "<!-- USER OPENEMENING OVERRIDE: |$VU_vicidial_recording_override|$campaign_recording| -->\n";
				}
			...
			$row=mysql_fetch_row($rslt);
			   ...
			   print "<!-- $campaign_leads_to_call - Leads nog te bellen in de hopper -->\n";
...
if (!$authphone)
	{
	echo "<title>VICIDIAL web client: Telefoon Inloggen Error</title>\n";
	echo "</head>\n";
	echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
	echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\"> Tijdslot</A><BR>\n";
	echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
### logged in and use that phone login account
	...
	if ( (strlen($park_ext)>0) && (strlen($park_file_name)>0) )
		...
		print "<!-- CAMPAGNE CUSTOM PARKEEREREN:  |$VICIDiaL_park_on_extension|$VICIDiaL_park_on_filename| -->\n";
		}
		print "<!-- CAMPAGNE DEFAULT PARKEEREREN: |$VICIDiaL_park_on_extension|$VICIDiaL_park_on_filename| -->\n";
	...
	if (strlen($web_form_address)>0)
		...
		print "<!-- CAMPAGNE CUSTOM WEB FORMULIER:   |$VICIDiaL_web_form_address| -->\n";
		}
	else
		{
	...
	if ($allow_closers=="Y")
		...
		print "<!-- CAMPAGNE STAAT CLOSERS TOE:    |$VICIDiaL_allow_closers| -->\n";
		}
	else
		{
		$VICIDiaL_allow_closers = 0;
		print "<!-- CAMPAGNE STAAT GEEN CLOSERS TOE: |$VICIDiaL_allow_closers| -->\n";
		}
	...
	if ( ($campaign_allow_inbound == 'Y') || ($campaign_leads_to_call > 0) || (ereg('Y',$no_hopper_leads_logins)) )
		...
		if ($prev_login_ct > 0)
			{print "<!-- GEBRUIK VORIGE MEETME KAMER - $session_id - $NOW_TIME - $SIP_user -->\n";}
		else
			{
			...
			print "<!-- GEBRUIK NIEUWE MEETME KAMER - $session_id - $NOW_TIME - $SIP_user -->\n";
			}
		...
		print "<!-- oude QUEUE en INKOMEND gedraaide list:   |$affected_rows| -->\n";
		...
		$affected_rows = mysql_affected_rows($link);
		print "<!-- oude QUEUE en INKOMEND gedraaide hopper: |$affected_rows| -->\n";
		...
		$affected_rows = mysql_affected_rows($link);
		print "<!-- oude vicidia_live_agents records opgeschoond: |$affected_rows| -->\n";
	...
	#	print "<B>You have logged in as user: $VD_login op phone: $SIP_user naar campagne: $VD_campaign</B><BR>\n";
		...
		if ($auto_dial_level > 0)
			{
			print "<!-- campagne staat op aut_dial_level: $auto_dial_level -->\n";
			...
			$affected_rows = mysql_affected_rows($link);
			print "<!-- nieuwe vicidial_live_agents record toegevoegd: |$affected_rows| -->\n";
...
?>
	...
	var wrapup_seconden = '<? echo $wrapup_seconden ?>';
...
// filter manual dialstring and pass op to originate call
	function SendManualDial(taskFromConf)
		...
		if (taskFromConf == 'YES')
			...
			document.getElementById("DialWithCustomer").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_dialwithcustomer_OFF.gif\" border=0 alt=\"Bel Met Klant\"></a>";

			document.getElementById("ParkCustomerDial").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_parkcustomerdial_OFF.gif\" border=0 alt=\"Parkeer Klant Gesprek\"></a>";
...
// ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ  above section working op API functions
						...
						if ( (conf_channels_xtra_display == 1) || (conf_channels_xtra_display == 0) )
							...
							if (live_conf_calls > 0)
								...
								agentphonelive=0;
								var conv_start=-1;
								var live_conf_HTML = "<font face=\"Arial,Helvetica\"><B>LIVE GESPREKKEN IN JOUW SESSIE:</B></font><BR><TABLE WIDTH=<?=$SDwidth ?>><TR BGCOLOR=#E6E6E6><TD><font class=\"log_title\">#</TD><TD><font class=\"log_title\">REMOTE KANAAL</TD><TD><font class=\"log_title\">OPHANGEN</TD><TD><font class=\"log_title\">VOLUME</TD></TR>";
			...
			if (xmlhttp) 
				...
				xmlhttp.onreadystatechange = function() 
					...
					//	alert(xmlhttp.responseText);
						...
						var loop_ct=0;
						var conv_start=0;
						var CB_HTML = "<table width=610><tr bgcolor=#E6E6E6><td><font class=\"log_title\">#</td><td><font class=\"log_title\"> BELBACK DATE/TIME</td><td><font class=\"log_title\">NUMMER</td><td><font class=\"log_title\">NAME</td><td><font class=\"log_title\">  STATUS</td><td align=right><font class=\"log_title\">CAMPAIGN</td><td><font class=\"log_title\">LAST DATUM\/TIJD GESPREK</td><td align=left><font class=\"log_title\"> COMMENTS</td></tr>"
					...
					//		alert("channel not found yet:\n" + campaign);
						...
						if (taskCheckOR == 'YES')
							...
							if ( (XDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') && (MD_ring_secondS < 10) )
								...
								document.getElementById("Leave3WayCall").innerHTML ="<a href=\"#\" onclick=\"leave_3way_call('FIRST');return false;\"><IMG SRC=\"../agc/images/vdc_XB_leave3waycall.gif\" border=0 alt=\"VERLAAT 3-WAY GESPREK\"></a>";

								document.getElementById("DialWithCustomer").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_dialwithcustomer_OFF.gif\" border=0 alt=\"Bel Met Klant\">";

								document.getElementById("ParkCustomerDial").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_parkcustomerdial_OFF.gif\" border=0 alt=\"Parkeer Klant Gesprek\">";

								document.getElementById("HangupXferLine").innerHTML ="<a href=\"#\" onclick=\"xfercall_send_hangup();return false;\"><IMG SRC=\"../agc/images/vdc_XB_hangupxferline.gif\" border=0 alt=\"Ophangen Xfer Lijn\"></a>";

								document.getElementById("HangupBothLines").innerHTML ="<a href=\"#\" onclick=\"bothcall_send_hangup();return false;\"><IMG SRC=\"../agc/images/vdc_XB_hangupbothlines.gif\" border=0 alt=\"Ophangen Beide Lijnen\"></a>";
							...
							if ( (MDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') )
								...
								document.getElementById("LocalCloser").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRLOCAL','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_XB_localcloser.gif\" border=0 alt=\"LOKALE CLOSER\"></a>";

								document.getElementById("DialBlindTransfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRBLIND','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_XB_blindtransfer.gif\" border=0 alt=\"Bel Blinde Transfer\"></a>";
					...
					//	alert(xmlhttprequestcheckauto.responseText);
						...
						//	DialLog("start");
							...
							if (VDIC_data_VDIG[1].length > 0)
								...
								document.getElementById("MainStatuSSpan").innerHTML = " Inkomend: " + status_display_number + " Group- " + VDIC_data_VDIG[1] + " &nbsp; " + VDIC_fronter; 
								}
							...
							if (alert_enabled=='ON')
								{
								alert(" Inkomend: " + status_display_number + "\n Group- " + VDIC_data_VDIG[1] + " &nbsp; " + VDIC_fronter);
								}
							}
						else
							{
...
// Send Hangup command for customer call connected to the conference now to Manager
	function dialedcall_send_hangup(dispowindow,hotkeysused,altdispo) 
		...
		//  DEACTIVATE CHANNEL-DEPENDANT BUTTONS AND VARIABLES
			...
			document.getElementById("LocalCloser").innerHTML = "<IMG SRC=\"../agc/images/vdc_XB_localcloser_OFF.gif\" border=0 alt=\"LOKALE CLOSER\">";
			document.getElementById("DialBlindTransfer").innerHTML = "<IMG SRC=\"../agc/images/vdc_XB_blindtransfer_OFF.gif\" border=0 alt=\"Bel Blinde Transfer\">";
...
// Send Hangup command for 3rd party call connected to the conference now to Manager
	function xfercall_send_hangup() 
		...
		//	document.getElementById("Leave3WayCall").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_leave3waycall_OFF.gif\" border=0 alt=\"VERLAAT 3-WAY GESPREK\">";
			...
			document.getElementById("DialWithCustomer").innerHTML ="<a href=\"#\" onclick=\"SendManualDial('YES');return false;\"><IMG SRC=\"../agc/images/vdc_XB_dialwithcustomer.gif\" border=0 alt=\"Bel Met Klant\"></a>";

			document.getElementById("ParkCustomerDial").innerHTML ="<a href=\"#\" onclick=\"xfer_park_dial();return false;\"><IMG SRC=\"../agc/images/vdc_XB_parkcustomerdial.gif\" border=0 alt=\"Parkeer Klant Gesprek\"></a>";

			document.getElementById("HangupXferLine").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_hangupxferline_OFF.gif\" border=0 alt=\"Ophangen Xfer Lijn\">";
...
// Generate the Closer In Group Chooser panel
	function CloserSelectContent_create()
		...
		if (VU_agent_choose_ingroups == '1')
			{
			var live_CSC_HTML = "<table cellpadding=5 cellspacing=5 width=500><tr><td><B>GROEP NIET GESELECTEERD</B></td><td><B>GESELECTEERDE GROEPEN</B></td></tr><tr><td bgcolor=\"#99FF99\" height=300 width=240 valign=top><font class=\"log_text\"><span id=CloserSelectAdd>";
			var loop_ct = 0;
	...
	function LogouT(tempreason)
		...
		if (MD_channel_look==1)
			...
			if (VD_live_customer_call==1)
				...
				document.getElementById("LogouTBoxLink").innerHTML = logout_content + "<a href=\"" + agcPAGE + "?relogin=YES&session_epoch=" + epoch_sec + "&session_id=" + session_id + "&session_name=" + session_name + "&VD_login=" + user + "&VD_campaign=" + campaign + "&phone_login=" + phone_login + "&phone_pass=" + phone_pass + "&VD_pass=" + pass + "\">KLIK HIER OM OPNIEUW IN TE LOGGEN</a>\n";
	...
	function start_all_refresh()
		...
		//	if ( (agentcall_manual != '1') && (starting_dial_level > 0) )
			...
			if (open_dispo_screen==1)
				{
				wrapup_counter=0;
				if (wrapup_seconden > 0)	
					{
					showDiv('WrapupBox');
					document.getElementById("WrapupTimer").innerHTML = wrapup_seconden;
					wrapup_waiting=1;
					}
				CustomerData_update();
			...
			if (WaitingForNextStep==0)
				...
				if (wrapup_seconden > 0)	
					{
					document.getElementById("WrapupTimer").innerHTML = (wrapup_seconden - wrapup_counter);
					wrapup_counter++;
					if ( (wrapup_counter > wrapup_seconden) && (document.getElementById("WrapupBox").style.visibility == 'visible') )
						{
						wrapup_waiting=0;
						hideDiv('WrapupBox');
						if (document.vicidial_form.DispoSelectStop.checked==true)
...
<?	echo "Ingelogged als: $VD_login op Telefoon: $SIP_user naar campagne: $VD_campaign&nbsp; \n"; ?>
...
<?	echo "<a href=\"#\" onclick=\"LogouT('NORMAL');return false;\">UITLOGGEN</a>\n"; ?>
</TD></TR></TABLE>
</SPAN>
...
<TD WIDTH=<?=$HSwidth ?> VALIGN=MIDDLE ALIGN=CENTER><font class="body_text"> &nbsp; <span id=status>LIVE</span> &nbsp; &nbsp; sessie ID: <span id=sessionIDspan></span> &nbsp; &nbsp; <span id=AgentStatusCalls></span></TD>
...
<span style="position:absolute;left:0px;top:0px;z-index:3;" id="WelkomBoxA">
    <table border=0 bgcolor="#FFFFFF" width=<?=$CAwidth ?> height=<?=$HKwidth ?>><TR><TD align=center><BR><span id="WelkomBoxAt">VICIDIAL</span></TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:<?=$SCwidth ?>px;top:49px;z-index:18;" id="SecondSspan"><font class="body_text"> seconden: 
...
<span style="position:absolute;left:0px;top:0px;z-index:39;" id="CallBacKsLisTBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=460><TR><TD align=center VALIGN=top> TERUGBELLERS VOOR AGENT <? echo $VD_login ?>:<BR>Klik op een terugbeller om de klant nu terug te bellen. Als je hierop klikt, wordt deze verwijderd uit de lijst.
	<BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:40;" id="NeWManuaLDiaLBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=460><TR><TD align=center VALIGN=top> NIEUWE HANDMATIGE LEAD VOOR <? echo "$VD_login in campaign $VD_campaign" ?>:<BR><BR>Voer de informatie voor de nieuwe lead hieronder in.
	<BR>
	<? 
...
<span style="position:absolute;left:5px;top:<?=$HTheight ?>px;z-index:21;" id="TransferMain">
	...
	<font class="body_text">
	 <IMG SRC="../agc/images/vdc_XB_header.gif" border=0 alt="Transfer- Conferentie"> &nbsp; &nbsp; &nbsp; &nbsp; <span id="XfeRDiaLGrouPSelecteD"></span> &nbsp; &nbsp; <span id="XfeRCID"></span><BR>
	...
	<span STYLE="background-color: #CCCCCC" id="LocalCloser"><IMG SRC="../agc/images/vdc_XB_localcloser_OFF.gif" border=0 alt="LOKALE CLOSER"></span> 
	<span STYLE="background-color: #CCCCCC" id="HangupXferLine"><IMG SRC="../agc/images/vdc_XB_hangupxferline_OFF.gif" border=0 alt="Ophangen Xfer Lijn"></span>
	<span STYLE="background-color: #CCCCCC" id="HangupBothLines"><a href="#" onclick="bothcall_send_hangup();return false;"><IMG SRC="../agc/images/vdc_XB_hangupbothlines.gif" border=0 alt="Ophangen Beide Lijnen"></a></span>

	<BR>

	<IMG SRC="../agc/images/vdc_XB_number.gif" border=0 alt="Nummer om te bellen"> <input type=text size=15 name=xfernumber maxlength=25 class="cust_form"> &nbsp; 
	...
	<span STYLE="background-color: #CCCCCC" id="Leave3WayCall"><a href="#" onclick="leave_3way_call('FIRST');return false;"><IMG SRC="../agc/images/vdc_XB_leave3waycall.gif" border=0 alt="VERLAAT 3-WAY GESPREK"></a></span> 
	<span STYLE="background-color: #CCCCCC" id="DialBlindTransfer"><IMG SRC="../agc/images/vdc_XB_blindtransfer_OFF.gif" border=0 alt="Bel Blinde Transfer"></span>
	...
	<IMG SRC="../agc/images/vdc_XB_seconden.gif" border=0 alt="seconden"> <input type=text size=2 name=xferlength maxlength=4 class="cust_form"> &nbsp; 
	...
	<span STYLE="background-color: #CCCCCC" id="DialWithCustomer"><a href="#" onclick="SendManualDial('YES');return false;"><IMG SRC="../agc/images/vdc_XB_dialwithcustomer.gif" border=0 alt="Bel Met Klant"></a></span> 
	<span STYLE="background-color: #CCCCCC" id="ParkCustomerDial"><a href="#" onclick="xfer_park_dial();return false;"><IMG SRC="../agc/images/vdc_XB_parkcustomerdial.gif" border=0 alt="Parkeer Klant Gesprek"></a></span> 
...
<span style="position:absolute;left:0px;top:12px;z-index:28;" id="NoneInSessionBox">
    <table border=1 bgcolor="#CCFFFF" width=<?=$CAwidth ?> height=500><TR><TD align=center> Er is niemand in jouw sessie: <span id="NoneInSessionID"></span><BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:29;" id="CustomerGoneBox">
    <table border=1 bgcolor="#CCFFFF" width=<?=$CAwidth ?> height=500><TR><TD align=center> Klant heeft opgehangen: <span id="CustomerGoneChanneL"></span><BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:30;" id="WrapupBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=550><TR><TD align=center> Call Wrapup: <span id="WrapupTimer"></span> seconden over in wrapup<BR><BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:31;" id="AgenTDisablEBoX">
    <table border=1 bgcolor="#FFFFFF" width=<?=$CAwidth ?> height=500><TR><TD align=center>Jouw sessie is disabled<BR><a href="#" onclick="LogouT('DISABLED');return false;">UITLOGGEN</a><BR><BR><a href="#" onclick="hideDiv('AgenTDisablEBoX');return false;">Ga Terug</a>
</TD></TR></TABLE>
</span>

<span style="position:absolute;left:0px;top:0px;z-index:32;" id="SysteMDisablEBoX">
    <table border=1 bgcolor="#FFFFFF" width=<?=$CAwidth ?> height=500><TR><TD align=center>Er is een tijd synchronisatie probleem met uw systeem, kunt u uw systeembeheerder<BR><BR><BR><a href="#" onclick="hideDiv('SysteMDisablEBoX');return false;">Ga Terug</a>
</TD></TR></TABLE>
</span>

<span style="position:absolute;left:0px;top:0px;z-index:33;" id="LogouTBox">
    <table border=1 bgcolor="#FFFFFF" width=<?=$CAwidth ?> height=500><TR><TD align=center><BR><span id="LogouTBoxLink">UITLOGGEN</span></TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:36;" id="DispoButtonHideC">
    <table border=0 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=47><TR><TD align=center VALIGN=top>Alle aanpassingen gedaan aan onderstaande klant informatie wordt niet doorgevoerd. Je moet de klant informatie aanpassen VOORDAT je VOLGENDE LEAD kiest. </TD></TR></TABLE>
</span>

<span style="position:absolute;left:0px;top:0px;z-index:37;" id="DispoSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=460><TR><TD align=center VALIGN=top> DISPOSITIONEER GESPREK :<span id="DispoSelectPhonE"></span> &nbsp; &nbsp; &nbsp; <span id="DispoSelectHAspan"><a href="#" onclick="DispoHanguPAgaiN()">Opnieuw Ophangen</a></span> &nbsp; &nbsp; &nbsp; <span id="DispoSelectMaxMin"><a href="#" onclick="DispoMinimize()"> minimaliseer </a></span><BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:43;" id="PauseCodeSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=460><TR><TD align=center VALIGN=top> SELECTEER EEN PAUZE CODE :<BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:44;" id="GroupAliasSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=460><TR><TD align=center VALIGN=top> KIES EEN GROEP ALIAS :<BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:38;" id="CallBackSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=460><TR><TD align=center VALIGN=top> Selecteer een terugbel datum :<span id="CallBackDatE"></span><BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:41;" id="CloserSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=460><TR><TD align=center VALIGN=top> CLOSER INKOMENDE GROEP SELECTIE <BR>
...
<span style="position:absolute;left:<?=$HKwidth ?>px;top:<?=$HKheight ?>px;z-index:16;" id="hotkeysdisplay"><a href="#" onMouseOver="HotKeys('ON')"><IMG SRC="../agc/images/vdc_XB_hotkeysactive_OFF.gif" border=0 alt="HOT KEYS NIET AKTIEF"></a></span>
<? } ?>


<span style="position:absolute;left:0px;top:<?=$HKheight ?>px;z-index:15;" id="MaiNfooterspan">
<table BGCOLOR="<? echo $MAIN_COLOR ?>" id="MaiNfooter" width=<?=$MNwidth ?>><tr height=32><td height=32><font face="Arial,Helvetica" size=1>VICIDIAL web-client versie: <? echo $version ?> &nbsp; &nbsp; BUILD: <? echo $build ?> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Server: <? echo $server_ip ?>  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;</font><BR>
<font class="body_small"><span id="busycallsdisplay"><a href="#"  onclick="conf_channels_detail('SHOW');">Toon conferentie gesprek kanaal informatie</a><BR><BR>&nbsp;</span></font></td><td align=right height=32>
</td></tr>
...
<span STYLE="background-color: #CCCCCC" id="RecorDControl"><a href="#" onclick="conf_send_recording('MonitorConf',session_id,'');return false;"><IMG SRC="../agc/images/vdc_LB_startrecording.gif" border=0 alt="Start Opnemen"></a></span><BR>
...
<span STYLE="background-color: #CCCCCC" id="ParkControl"><IMG SRC="../agc/images/vdc_LB_parkcall_OFF.gif" border=0 alt="Parkeer Gesprek"></span><BR>
<span STYLE="background-color: #CCCCCC" id="XferControl"><IMG SRC="../agc/images/vdc_LB_transferconf_OFF.gif" border=0 alt="Transfer- Conferentie"></span><BR>
...
<div class="text_input" id="SendDTMFdiv"><span STYLE="background-color: #CCCCCC" id="SendDTMF"><a href="#" onclick="SendConfDTMF(session_id);return false;"><IMG SRC="../agc/images/vdc_LB_senddtmf.gif" border=0 alt="Stuur DTMF" align=bottom></a>  <input type=text size=5 name=conf_dtmf class="cust_form" value="" maxlength=50></div></span><BR>
</center>
</font>
</td>
