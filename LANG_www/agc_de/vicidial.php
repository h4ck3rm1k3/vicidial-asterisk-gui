...
$SCwidth =  ($MASTERwidth + 230);	# 670 - live call Sekunden counter
...
if ($user_login_first == 1)
{
	...
	{
	echo "<title>VICIDIAL web client: Kampagne Login</title>\n";
	echo "</head>\n";
	echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
	echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\"> Stechuhr</A><BR>\n";
	echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
if ( (strlen($phone_login)<2) or (strlen($phone_pass)<2) )
{
echo "<title>VICIDIAL web client:  Telefon Login</title>\n";
echo "</head>\n";
echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\"> Stechuhr</A><BR>\n";
echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
	...
	if($auth>0)
		...
		if ( ( (ereg("START|ALL",$shift_enforcement)) and (!ereg("OFF",$VU_agent_shift_enforcement_override)) ) or (ereg("START|ALL",$VU_agent_shift_enforcement_override)) )
			...
			if ( ($shift_ok < 1) and ($VU_shift_override_flag < 1) and ($VDloginDISPLAY > 0) )
				...
				$VDdisplayMESSAGE.= "Manager Passwort: <INPUT TYPE=PASSWORD NAME=\"MGR_pass$loginDATE\" SIZE=10 maxlength=20><br>\n";
		...
		if ( (!eregi(" $VD_campaign ",$LOGallowed_campaigns)) and (!eregi("ALL-CAMPAIGNS",$LOGallowed_campaigns)) )
			{
			echo "<title>VICIDIAL web client: VICIDIAL Kampagne Login</title>\n";
			echo "</head>\n";
			echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
			echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\"> Stechuhr</A><BR>\n";
			echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
if (!$authphone)
	{
	echo "<title>VICIDIAL web client: Telefon Login Error</title>\n";
	echo "</head>\n";
	echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
	echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\"> Stechuhr</A><BR>\n";
	echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
### logged in and use that phone login account
	...
	if ($allow_closers=="Y")
		...
		print "<!-- KAMPAGNE ERLAUBT NACHFRAGE:    |$VICIDiaL_allow_closers| -->\n";
		}
	else
		{
		$VICIDiaL_allow_closers = 0;
		print "<!-- KAMPAGNE ERLAUBT KEINE NACHFRAGE: |$VICIDiaL_allow_closers| -->\n";
		}
	...
	if ( ($campaign_allow_inbound == 'Y') || ($campaign_leads_to_call > 0) || (ereg('Y',$no_hopper_leads_logins)) )
		...
		if ($prev_login_ct > 0)
			{print "<!-- VERWENDE VORHERIGEN MEETME RAUM - $session_id - $NOW_TIME - $SIP_user -->\n";}
		else
			{
			...
			print "<!-- VERWENDE NEUEN MEETME RAUM - $session_id - $NOW_TIME - $SIP_user -->\n";
			}
...
// filter manual dialstring and pass an to originate call
	function SendManualDial(taskFromConf)
		...
		if (taskFromConf == 'YES')
			...
			document.getElementById("DialWithCustomer").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_dialwithcustomer_OFF.gif\" border=0 alt=\"mit Kunde wählen\"></a>";

			document.getElementById("ParkCustomerDial").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_parkcustomerdial_OFF.gif\" border=0 alt=\"Kundenanruf parken\"></a>";
...
// ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ  above section working an API functions
						...
						if ( (conf_channels_xtra_display == 1) || (conf_channels_xtra_display == 0) )
							...
							if (live_conf_calls > 0)
								...
								agentphonelive=0;
								var conv_start=-1;
								var live_conf_HTML = "<font face=\"Arial,Helvetica\"><B>LIVE ANRUFE IN IHRER SITZUNG:</B></font><BR><TABLE WIDTH=<?=$SDwidth ?>><TR BGCOLOR=#E6E6E6><TD><font class=\"log_title\">#</TD><TD><font class=\"log_title\">ENTFERNTE LEITUNG</TD><TD><font class=\"log_title\">AUFLEGEN</TD><TD><font class=\"log_title\">LAUTSTÄRKE</TD></TR>";
					...
					//		alert("channel not found yet:\n" + campaign);
						...
						if (taskCheckOR == 'YES')
							...
							if ( (XDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') && (MD_ring_secondS < 10) )
								...
								document.getElementById("Leave3WayCall").innerHTML ="<a href=\"#\" onclick=\"leave_3way_call('FIRST');return false;\"><IMG SRC=\"../agc/images/vdc_XB_leave3waycall.gif\" border=0 alt=\"NACHFRAGE VERLASSEN\"></a>";

								document.getElementById("DialWithCustomer").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_dialwithcustomer_OFF.gif\" border=0 alt=\"mit Kunde wählen\">";
							...
							if ( (MDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') )
								...
								document.getElementById("LocalCloser").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRLOCAL','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_XB_localcloser.gif\" border=0 alt=\"LOKALE SUPERVISOR NACHFRAGE\"></a>";

								document.getElementById("DialBlindTransfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRBLIND','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_XB_blindtransfer.gif\" border=0 alt=\"Verbinden ohne Nachfrage\"></a>";
				...
				xmlhttprequestcheckauto.onreadystatechange = function() 
					...
					//	alert(xmlhttprequestcheckauto.responseText);
						...
						//	DialLog("start");
							...
							if (alert_enabled=='ON')
								{
								alert(" Eingehend: " + status_display_number + "\n Group- " + VDIC_data_VDIG[1] + " &nbsp; " + VDIC_fronter);
								}
							}
						else
							{
...
// Send Hangup command for customer call connected to the conference now to Manager
	function dialedcall_send_hangup(dispowindow,hotkeysused,altdispo) 
		...
		//  DEACTIVATE CHANNEL-DEPENDANT BUTTONS AND VARIABLES
			...
			document.getElementById("LocalCloser").innerHTML = "<IMG SRC=\"../agc/images/vdc_XB_localcloser_OFF.gif\" border=0 alt=\"LOKALE SUPERVISOR NACHFRAGE\">";
			document.getElementById("DialBlindTransfer").innerHTML = "<IMG SRC=\"../agc/images/vdc_XB_blindtransfer_OFF.gif\" border=0 alt=\"Verbinden ohne Nachfrage\">";
...
// Send Hangup command for 3rd party call connected to the conference now to Manager
	function xfercall_send_hangup() 
		...
		//	document.getElementById("Leave3WayCall").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_leave3waycall_OFF.gif\" border=0 alt=\"NACHFRAGE VERLASSEN\">";
			...
			document.getElementById("DialWithCustomer").innerHTML ="<a href=\"#\" onclick=\"SendManualDial('YES');return false;\"><IMG SRC=\"../agc/images/vdc_XB_dialwithcustomer.gif\" border=0 alt=\"mit Kunde wählen\"></a>";
			...
			document.getElementById("HangupXferLine").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_hangupxferline_OFF.gif\" border=0 alt=\"Nachfrage auflegen\">";
	...
	function LogouT(tempreason)
		...
		if (MD_channel_look==1)
			...
			if (VD_live_customer_call==1)
				...
				document.getElementById("LogouTBoxLink").innerHTML = logout_content + "<a href=\"" + agcPAGE + "?relogin=YES&session_epoch=" + epoch_sec + "&session_id=" + session_id + "&session_name=" + session_name + "&VD_login=" + user + "&VD_campaign=" + campaign + "&phone_login=" + phone_login + "&phone_pass=" + phone_pass + "&VD_pass=" + pass + "\">KLICKEN SIE HIER, UM SICH WIEDER ANZUMELDEN</a>\n";
...
<span style="position:absolute;left:0px;top:0px;z-index:3;" id="WillkommenBoxA">
    <table border=0 bgcolor="#FFFFFF" width=<?=$CAwidth ?> height=<?=$HKwidth ?>><TR><TD align=center><BR><span id="WillkommenBoxAt">VICIDIAL</span></TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:<?=$SCwidth ?>px;top:49px;z-index:18;" id="SecondSspan"><font class="body_text"> Sekunden: 
...
<span style="position:absolute;left:5px;top:<?=$HTheight ?>px;z-index:21;" id="TransferMain">
	...
	<font class="body_text">
	 <IMG SRC="../agc/images/vdc_XB_header.gif" border=0 alt="Transfer - Konferenz"> &nbsp; &nbsp; &nbsp; &nbsp; <span id="XfeRDiaLGrouPSelecteD"></span> &nbsp; &nbsp; <span id="XfeRCID"></span><BR>
	...
	<span STYLE="background-color: #CCCCCC" id="LocalCloser"><IMG SRC="../agc/images/vdc_XB_localcloser_OFF.gif" border=0 alt="LOKALE SUPERVISOR NACHFRAGE"></span> 
	<span STYLE="background-color: #CCCCCC" id="HangupXferLine"><IMG SRC="../agc/images/vdc_XB_hangupxferline_OFF.gif" border=0 alt="Nachfrage auflegen"></span>
	...
	<span STYLE="background-color: #CCCCCC" id="Leave3WayCall"><a href="#" onclick="leave_3way_call('FIRST');return false;"><IMG SRC="../agc/images/vdc_XB_leave3waycall.gif" border=0 alt="NACHFRAGE VERLASSEN"></a></span> 
	<span STYLE="background-color: #CCCCCC" id="DialBlindTransfer"><IMG SRC="../agc/images/vdc_XB_blindtransfer_OFF.gif" border=0 alt="Verbinden ohne Nachfrage"></span>
	...
	<span STYLE="background-color: #CCCCCC" id="DialWithCustomer"><a href="#" onclick="SendManualDial('YES');return false;"><IMG SRC="../agc/images/vdc_XB_dialwithcustomer.gif" border=0 alt="mit Kunde wählen"></a></span> 
...
<span style="position:absolute;left:0px;top:0px;z-index:31;" id="AgenTDisablEBoX">
    <table border=1 bgcolor="#FFFFFF" width=<?=$CAwidth ?> height=500><TR><TD align=center>Ihre Sitzung ist gesperrt worden<BR><a href="#" onclick="LogouT('DISABLED');return false;">LOGOUT</a><BR><BR><a href="#" onclick="hideDiv('AgenTDisablEBoX');return false;">Zurück</a>
</TD></TR></TABLE>
</span>

<span style="position:absolute;left:0px;top:0px;z-index:32;" id="SysteMDisablEBoX">
    <table border=1 bgcolor="#FFFFFF" width=<?=$CAwidth ?> height=500><TR><TD align=center>Es ist eine Zeit-Synchronisation Problem mit Ihrem System haben, wenden Sie sich bitte sagen Sie sich an Ihren Systemadministrator<BR><BR><BR><a href="#" onclick="hideDiv('SysteMDisablEBoX');return false;">Zurück</a>
</TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:37;" id="DispoSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=460><TR><TD align=center VALIGN=top> ANRUF ABSCHLIESSEN :<span id="DispoSelectPhonE"></span> &nbsp; &nbsp; &nbsp; <span id="DispoSelectHAspan"><a href="#" onclick="DispoHanguPAgaiN()">Wieder Auflegen</a></span> &nbsp; &nbsp; &nbsp; <span id="DispoSelectMaxMin"><a href="#" onclick="DispoMinimize()"> minimieren </a></span><BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:43;" id="PauseCodeSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=460><TR><TD align=center VALIGN=top> WÄHLEN Sie Einen PAUSE CODE :<BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:44;" id="GroupAliasSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=460><TR><TD align=center VALIGN=top> Wählen Sie eine Gruppe ALIAS :<BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:38;" id="CallBackSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=460><TR><TD align=center VALIGN=top> Wählen Sie ein Rückrufdatum :<span id="CallBackDatE"></span><BR>
...
<span style="position:absolute;left:<?=$HKwidth ?>px;top:<?=$HKheight ?>px;z-index:16;" id="hotkeysdisplay"><a href="#" onMouseOver="HotKeys('ON')"><IMG SRC="../agc/images/vdc_XB_hotkeysactive_OFF_de.gif" border=0 alt="TASTATURKÜRZEL DEAKTIVIERT"></a></span>
<? } ?>
...
<font class="body_small"><span id="busycallsdisplay"><a href="#"  onclick="conf_channels_detail('SHOW');">Konferenz Informationen anzeigen</a><BR><BR>&nbsp;</span></font></td><td align=right height=32>
</td></tr>
...
<span STYLE="background-color: #CCCCCC" id="RecorDControl"><a href="#" onclick="conf_send_recording('MonitorConf',session_id,'');return false;"><IMG SRC="../agc/images/vdc_LB_startrecording_de.gif" border=0 alt="Starte Aufnahme"></a></span><BR>
...
<span STYLE="background-color: #CCCCCC" id="ParkControl"><IMG SRC="../agc/images/vdc_LB_parkcall_OFF_de.gif" border=0 alt="Parke Anruf"></span><BR>
<span STYLE="background-color: #CCCCCC" id="XferControl"><IMG SRC="../agc/images/vdc_LB_transferconf_OFF_de.gif" border=0 alt="Transfer - Konferenz"></span><BR>
