		...
		if ($auto_dial_level > 0)
			...
			if ($affected_rows > 0) 
				...
				if ($enable_queuemetrics_logging > 0)
					...
					$stmt = "INSERT INTO queue_log SET partition='P01',time_id='$StarTtime',call_id='NONE',queue='NONE',agent='Agent/$user',verb='PRZERWAALL',serverid='$queuemetrics_log_id';";
					if ($DB) {echo "$stmt\n";}
					if ($non_latin > 0) {$rslt=mysql_query("SET NAMES 'UTF8'");}
					$rslt=mysql_query($stmt, $linkB);
					$affected_rows = mysql_affected_rows($linkB);
...
if ( (strlen($campaign)<1) || (strlen($conf_exten)<1) )
	...
	if ( ($VDpr_ct > 0) and (strlen($row[3]<5)) and (strlen($row[4]<5)) )
		...
		if ($vla_delete > 0) 
			...
			#	$stmt = "INSERT INTO queue_log SET partition='P01',time_id='$StarTtime',call_id='NONE',queue='$campaign',agent='Agent/$user',verb='PRZERWA',serverid='1';";
			#	if ($DB) {echo "$stmt\n";}
			#	if ($non_latin > 0) {$rslt=mysql_query("SET NAMES 'UTF8'");}
			#	$rslt=mysql_query($stmt, $linkB);
			#	$affected_rows = mysql_affected_rows($linkB);
...
if ( ($ACTION == 'VDADpause') || ($ACTION == 'VDADready') )
{
	...
	if ($affected_rows > 0) 
		...
		if ($enable_queuemetrics_logging > 0)
			...
			if (ereg('PRZERWA',$stage)) {$QMstatus='PRZERWAALL';}
			$linkB=mysql_connect("$queuemetrics_server_ip", "$queuemetrics_login", "$queuemetrics_pass");
			mysql_select_db("$queuemetrics_dbname", $linkB);
...
if ($ACTION == 'PrzerwaCodeSubmit')
{
	...
	if ($affected_rows > 0) 
		...
		if ($enable_queuemetrics_logging > 0)
			...
			$stmt = "INSERT INTO queue_log SET partition='P01',time_id='$StarTtime',call_id='NONE',queue='$campaign',agent='Agent/$user',verb='PRZERWAREASON',serverid='$queuemetrics_log_id',data1='$status';";
			if ($DB) {echo "$stmt\n";}
			if ($non_latin > 0) {$rslt=mysql_query("SET NAMES 'UTF8'");}
			$rslt=mysql_query($stmt, $linkB);
			$affected_rows = mysql_affected_rows($linkB);
