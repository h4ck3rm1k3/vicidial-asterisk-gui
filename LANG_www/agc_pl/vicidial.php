...
$PreseT_DiaL_LinKs		= '1';	# set to 1 to show a WYBIERZ link for Dial Presets
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_pl/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Polski <img src=\"../agc/images/pl.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";			echo "</TR></TABLE>\n";
		...
		if($CAMPactive>0)
			...
			if ( (!ereg('DISABLED',$VU_vicidial_recording_override)) and ($VU_vicidial_recording > 0) )
				...
				print "<!-- USER NAGRYWAJING OVERRIDE: |$VU_vicidial_recording_override|$campaign_recording| -->\n";
				}
			...
			$row=mysql_fetch_row($rslt);
			   ...
			   print "<!-- $campaign_leads_to_call - pozostałych kontaktów oczekujących wykonania połączeń w zasobniku -->\n";
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_pl/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Polski <img src=\"../agc/images/pl.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
	...
	if ( (strlen($park_ext)>0) && (strlen($park_file_name)>0) )
		...
		print "<!-- KAMPANIA- ZAWIESZANIE ZDEFINIOWANE PRZEZ UŻYTKOWNIKA:  |$VICIDiaL_park_on_extension|$VICIDiaL_park_on_filename| -->\n";
		}
		print "<!-- KAMPANIA- DOMYŚLNE ZAWIESZANIE: |$VICIDiaL_park_on_extension|$VICIDiaL_park_on_filename| -->\n";
	...
	if (strlen($web_form_address)>0)
		...
		print "<!-- KAMPANIA- FORMULARZ WWW ZDEFINIOWANY PRZEZ UŻYTKOWNIKA:   |$VICIDiaL_web_form_address| -->\n";
		}
	else
		{
	...
	if ($allow_closers=="Y")
		...
		print "<!-- KAMPANIA ZEZWALA NA FINALIZUJĄCYCH:    |$VICIDiaL_allow_closers| -->\n";
		}
	else
		{
		$VICIDiaL_allow_closers = 0;
		print "<!-- KAMPANIA NIE ZEZWALA NA FINALIZUJĄCYCH: |$VICIDiaL_allow_closers| -->\n";
		}
	...
	if ( ($campaign_allow_inbound == 'Y') || ($campaign_leads_to_call > 0) || (ereg('Y',$no_hopper_leads_logins)) )
		...
		if ($prev_login_ct > 0)
			{print "<!-- UŻYWAM POPRZEDNIEGO MEETME - $session_id - $NOW_TIME - $SIP_user -->\n";}
		else
			{
			...
			if ($non_latin > 0) {$rslt=mysql_query("SET NAMES 'UTF8'");}
			$rslt=mysql_query($stmt, $link);
			print "<!-- UŻYWAM NOWEGO MEETME - $session_id - $NOW_TIME - $SIP_user -->\n";
		...
		print "<!-- stara Kolejka i Aktulanie wybierane numery powróciły na listę:   |$affected_rows| -->\n";
		...
		$rslt=mysql_query($stmt, $link);
		$affected_rows = mysql_affected_rows($link);
		print "<!-- stara Kolejka i Aktualnie wybrane numery powróciły do zasobnika: |$affected_rows| -->\n";
		...
		$rslt=mysql_query($stmt, $link);
		$affected_rows = mysql_affected_rows($link);
		print "<!-- stare rekordy vicidial_live_agents zostały wyczyszczone: |$affected_rows| -->\n";
	...
	#	print "<B>You have logged in as user: $VD_login na phone: $SIP_user do kampani: $VD_campaign</B><BR>\n";
		...
		if ($auto_dial_level > 0)
			{
			print "<!-- kampania jest ustawiona na auto_dial_level: $auto_dial_level -->\n";
			...
			if ($enable_queuemetrics_logging > 0)
				...
				$stmt = "INSERT INTO queue_log SET partition='P01',time_id='$StarTtimE',call_id='NONE',queue='NONE',agent='Agent/$VD_login',verb='PRZERWAALL',serverid='$queuemetrics_log_id';";
				if ($DB) {echo "$stmt\n";}
				if ($non_latin > 0) {$rslt=mysql_query("SET NAMES 'UTF8'");}
				$rslt=mysql_query($stmt, $linkB);
				$affected_rows = mysql_affected_rows($linkB);
				print "<!-- queue_log PRZERWA entry added: $VD_login|$affected_rows -->\n";
...
?>
	...
	var wrapup_sekundy = '<? echo $wrapup_sekundy ?>';
					...
					//	alert(xmlhttprequestcheckconf.responseText);
						...
						if ( (conf_channels_xtra_display == 1) || (conf_channels_xtra_display == 0) )
							...
							if (live_conf_calls > 0)
								...
								var conv_start=-1;
								var live_conf_HTML = "<font face=\"Arial,Helvetica\"><B>AKTYWNE ROZMOWY W TWOJEJ SESJI:</B></font><BR><TABLE WIDTH=<?=$SDwidth ?>><TR BGCOLOR=#E6E6E6><TD><font class=\"log_title\">#</TD><TD><font class=\"log_title\">KANAŁ ZDALNY</TD><TD><font class=\"log_title\">ROZŁĄCZ</TD><TD><font class=\"log_title\">VOLUME</TD></TR>";
								...
								while (loop_ct < live_conf_calls)
									...
									if ( (HidEMonitoRSessionS==1) && (channelfieldA.match(/68600/)) )
										...
										if (volumecontrol_active!=1)
											{
											live_conf_HTML = live_conf_HTML + "<tr bgcolor=\"" + row_color + "\"><td><font class=\"log_text\">" + loop_ct + "</td><td><font class=\"log_text\">" + channelfieldA + "</td><td><font class=\"log_text\"><a href=\"#\" onclick=\"livehangup_send_hangup('" + channelfieldA + "');return false;\">ROZŁĄCZ</a></td><td></td></tr>";
											}
										else
											{
				...
				//		alert("channel not found yet:\n" + campaign);
						...
						if (taskCheckOR == 'YES')
							...
							if ( (XDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') && (MD_ring_secondS < 10) )
								...
								document.getElementById("Leave3WayCall").innerHTML ="<a href=\"#\" onclick=\"leave_3way_call('FIRST');return false;\"><IMG SRC=\"../agc/images/vdc_XB_leave3waycall.gif\" border=0 alt=\"WYJDŹ Z ROZMOWY 3-STRONNEJ\"></a>";

								document.getElementById("DialWithCustomer").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_dialwithcustomer_OFF.gif\" border=0 alt=\"Połącz z klientem\">";

								document.getElementById("ParkCustomerDial").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_parkcustomerdial_OFF.gif\" border=0 alt=\"Zawieś połączenie z klientem\">";

								document.getElementById("HangupXferLine").innerHTML ="<a href=\"#\" onclick=\"xfercall_send_hangup();return false;\"><IMG SRC=\"../agc/images/vdc_XB_hangupxferline.gif\" border=0 alt=\"Rozłącz linię transferu\"></a>";

								document.getElementById("HangupBothLines").innerHTML ="<a href=\"#\" onclick=\"bothcall_send_hangup();return false;\"><IMG SRC=\"../agc/images/vdc_XB_hangupbothlines.gif\" border=0 alt=\"Rozłącz obie linie\"></a>";
							...
							if ( (MDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') )
								...
								document.getElementById("LocalCloser").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRLOCAL','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_XB_localcloser.gif\" border=0 alt=\"LOKALNY FINALIZUJĄCYCH\"></a>";

								document.getElementById("DialBlindTransfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRBLIND','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_XB_blindtransfer.gif\" border=0 alt=\"Wybierz numer do nieweryfikowanego transferu\"></a>";
								...
								// INSERT VICIWYBIERZ_LOG ENTRY FOR THIS Rozmowa PROCESS
								DialLog("start");
...
// $VICIWYBIERZ_web_QUERY_STRING =~ s/ /+/gi;
// $VICIWYBIERZ_web_QUERY_STRING =~ s/\`|\~|\:|\;|\#|\'|\"|\{|\}|\(|\)|\*|\^|\%|\$|\!|\%|\r|\t|\n//gi;
				...
				xmlhttprequestcheckauto.onreadystatechange = function() 
					...
					//		alert(xmlhttprequestcheckauto.responseText);
						...
						//	DialLog("start");
							...
							document.getElementById("MainStatuSSpan").innerHTML = " Przychodząca: " + status_display_number + " UID: " + CIDcheck + " &nbsp; " + VDIC_fronter; 
							...
							if (VDIC_data_VDIG[1].length > 0)
								...
								document.getElementById("MainStatuSSpan").innerHTML = " Przychodząca: " + status_display_number + " Group- " + VDIC_data_VDIG[1] + " &nbsp; " + VDIC_fronter; 
								}
...
// Send Hangup command for customer call connected to the conference now to Manager
	function dialedcall_send_hangup(dispowindow,hotkeysused,altdispo) 
		...
		//	UPDATE VICIWYBIERZ_LOG ENTRY FOR THIS Rozmowa PROCESS
			DialLog("end");
			if (dispowindow == 'NO')
				{
				open_dispo_screen=0;
		...
		//  DEACTIVATE CHANNEL-DEPENDANT BUTTONS AND VARIABLES
			...
			document.getElementById("LocalCloser").innerHTML = "<IMG SRC=\"../agc/images/vdc_XB_localcloser_OFF.gif\" border=0 alt=\"LOKALNY FINALIZUJĄCYCH\">";
			document.getElementById("DialBlindTransfer").innerHTML = "<IMG SRC=\"../agc/images/vdc_XB_blindtransfer_OFF.gif\" border=0 alt=\"Wybierz numer do nieweryfikowanego transferu\">";
...
// Send Hangup command for 3rd party call connected to the conference now to Manager
	function xfercall_send_hangup() 
		...
		//  DEACTIVATE CHANNEL-DEPENDANT BUTTONS AND VARIABLES
			...
			document.getElementById("DialWithCustomer").innerHTML ="<a href=\"#\" onclick=\"SendManualDial('YES');return false;\"><IMG SRC=\"../agc/images/vdc_XB_dialwithcustomer.gif\" border=0 alt=\"Połącz z klientem\"></a>";

			document.getElementById("ParkCustomerDial").innerHTML ="<a href=\"#\" onclick=\"xfer_park_dial();return false;\"><IMG SRC=\"../agc/images/vdc_XB_parkcustomerdial.gif\" border=0 alt=\"Zawieś połączenie z klientem\"></a>";

			document.getElementById("HangupXferLine").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_hangupxferline_OFF.gif\" border=0 alt=\"Rozłącz linię transferu\">";
...
// Generate the Przerwa Code Chooser panel
	function PauseCodeSelectContent_create()
		...
		if ( (AutoDialWaiting == 1) || (VD_live_customer_call==1) || (alt_dial_active==1) )
			...
			{
			showDiv('PrzerwaCodeSelectBox');
			WaitingForNextStep=1;
			PrzerwaCode_HTML = '';
			document.vicidial_form.PrzerwaCodeSelection.value = '';		
			var VD_pause_codes_ct_half = parseInt(VD_pause_codes_ct / 2);
			PrzerwaCode_HTML = "<table cellpadding=5 cellspacing=5 width=500><tr><td colspan=2><B> PRZERWA CODE</B></td></tr><tr><td bgcolor=\"#99FF99\" height=300 width=240 valign=top><font class=\"log_text\"><span id=PrzerwaCodeSelectA>";
			var loop_ct = 0;
			while (loop_ct < VD_pause_codes_ct)
				{
				PrzerwaCode_HTML = PrzerwaCode_HTML + "<font size=3 style=\"BACKGROUND-COLOR: #FFFFCC\"><b><a href=\"#\" onclick=\"PrzerwaCodeSelect_submit('" + VARpause_codes[loop_ct] + "');return false;\">" + VARpause_codes[loop_ct] + " - " + VARpause_code_names[loop_ct] + "</a></b></font><BR><BR>";
				loop_ct++;
				if (loop_ct == VD_pause_codes_ct_half) 
					{PrzerwaCode_HTML = PrzerwaCode_HTML + "</span></font></td><td bgcolor=\"#99FF99\" height=300 width=240 valign=top><font class=\"log_text\"><span id=PrzerwaCodeSelectB>";}
				}
			PrzerwaCode_HTML = PrzerwaCode_HTML + "</span></font></td></tr></table><BR><BR><font size=3 style=\"BACKGROUND-COLOR: #FFFFCC\"><b><a href=\"#\" onclick=\"PrzerwaCodeSelect_submit('');return false;\">Wróć</a>";
			document.getElementById("PrzerwaCodeSelectContent").innerHTML = PrzerwaCode_HTML;
			}
		}
...
// Submit the Przerwa Code 
	function PauseCodeSelect_submit(newpausecode)
		...
		if (xmlhttp) 
			{ 
			VMCpausecode_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&user=" + user + "&pass=" + pass  + "&ACTION=PrzerwaCodeSubmit&format=text&status=" + newpausecode + "&agent_log_id=" + agent_log_id + "&campaign=" + campaign + "&extension=" + extension + "&protocol=" + protocol + "&phone_ip=" + phone_ip + "&enable_sipsak_messages=" + enable_sipsak_messages;
			xmlhttp.open('POST', 'vdc_db_query.php'); 
			xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
...
// ################################################################################
// Pokaż message that there are no voice channels in the VICIWYBIERZ session
...
// Generate the Closer In Group Chooser panel
	function CloserSelectContent_create()
		...
		if (VU_agent_choose_ingroups == '1')
			{
			var live_CSC_HTML = "<table cellpadding=5 cellspacing=5 width=500><tr><td><B>GRUPY NIE WYBRANE</B></td><td><B>WYBRANE GRUPY</B></td></tr><tr><td bgcolor=\"#99FF99\" height=300 width=240 valign=top><font class=\"log_text\"><span id=CloserSelectAdd>";
			var loop_ct = 0;
	...
	function LogouT()
		...
		if (MD_channel_look==1)
			...
			if (VD_live_customer_call==1)
				...
				document.getElementById("LogouTBoxLink").innerHTML = "<a href=\"" + agcPAGE + "?relogin=YES&session_epoch=" + epoch_sec + "&session_id=" + session_id + "&session_name=" + session_name + "&VD_login=" + user + "&VD_campaign=" + campaign + "&phone_login=" + phone_login + "&phone_pass=" + phone_pass + "&VD_pass=" + pass + "\">KLINKNIJ TU, ABY PONOWNIE ZALOGOWAĆ SIĘ</a>\n";
...
// ################################################################################
// Move the Dispo frame out of the way and change the link to maksymalizuj
...
// ################################################################################
// Move the Dispo frame to the top and change the link to minimalizuj
...
// GLOBAL FUNCTIONS
	...
	function start_all_refresh()
		...
		//	if ( (agentcall_manual != '1') && (starting_dial_level > 0) )
			...
			if (agent_pause_codes_active=='Y')
				{
				document.getElementById("PrzerwaCodeLinkSpan").innerHTML = "<a href=\"#\" onclick=\"PrzerwaCodeSelectContent_create();return false;\">WCHODZĄ SZYFR PRZERWY</a>";
				}
			...
			if (open_dispo_screen==1)
				...
				if (wrapup_sekundy > 0)	
					{
					showDiv('WrapupBox');
					document.getElementById("WrapupTimer").innerHTML = wrapup_sekundy;
					wrapup_waiting=1;
					}
				CustomerData_update();
			...
			if (WaitingForNextStep==0)
				...
				if (wrapup_sekundy > 0)	
					{
					document.getElementById("WrapupTimer").innerHTML = (wrapup_sekundy - wrapup_counter);
					wrapup_counter++;
					if ( (wrapup_counter > wrapup_sekundy) && (document.getElementById("WrapupBox").style.visibility == 'visible') )
						{
						wrapup_waiting=0;
						hideDiv('WrapupBox');
						if (document.vicidial_form.DispoSelectStop.checked==true)
	...
	function clearDiv(divvar)
		...
		if (document.getElementById(divvar))
			...
			if (divvar == 'DiaLDiaLAltPhonE')
				{
				var buildDivHTML = "<font class=\"preview_text\"> <input type=checkbox name=DiaLAltPhonE size=1 value=\"0\"> WYBIERANIE DRUGIEGO NUMERU<BR></font>";
...
<?	echo "Zalogowany jako: $VD_login na Telefon: $SIP_user do kampani: $VD_campaign&nbsp; \n"; ?>
</TD><TD COLSPAN=3 VALIGN=TOP ALIGN=RIGHT><font class="body_text"><?	echo "<a href=\"#\" onclick=\"LogouT();return false;\">WYLOGUJ</a>\n"; ?>
</TD></TR></TABLE>
</SPAN>
...
<span style="position:absolute;left:0px;top:0px;z-index:3;" id="WitajBoxA">
    <table border=0 bgcolor="#FFFFFF" width=<?=$CAwidth ?> height=<?=$HKwidth ?>><TR><TD align=center><BR><span id="WitajBoxAt">VICIWYBIERZ</span></TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:500px;top:<?=$CBheight ?>px;z-index:14;" id="PrzerwaCodeButtons"><font class="body_text">
<span id="PrzerwaCodeLinkSpan"></span> <BR>
</font></span>
...
<span style="position:absolute;left:0px;top:0px;z-index:38;" id="CallBacKsLisTBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=460><TR><TD align=center VALIGN=top> INNE TERMINY DLA AGENTA <? echo $VD_login ?>:<BR>Kliknij poniżej na Inne terminy żeby teraz zadzwonić do klienta. Jeżeli klikniesz na rekordzie by zadzwonić, rekord zostanie usunięty z listy.
	<BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:39;" id="NeWManuaLDiaLBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=460><TR><TD align=center VALIGN=top> NOWE RĘCZNE WYBIERANIE NUMERU DLA <? echo "$VD_login in campaign $VD_campaign" ?>:<BR><BR>Wpisz poniżej informacje o kontakcie do którego chcesz wykonać połączenie.
	<BR>
	<? 
	...
	<td align=right colspan=2><BR><BR>Jeżeli chcesz wybrać numer tak aby NIE został dodany jako nowy kontakt, wpisz dokładny numer telefonu w polu Wybierz ten numer poniżej. By zakończyć połączenie musisz otworzyć ROZMOWY W TEJ SESJI (w dole ekranu), i zakończyć połączenie klinając na link jego kanału..<BR> &nbsp; </td>
	</tr><tr>
	...
	<BR>
	<a href="#" onclick="NeWManuaLDiaLCalLSubmiT();return false;">Wybierz teraz</a>
	 &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp; 
...
<span style="position:absolute;left:5px;top:<?=$HTheight ?>px;z-index:21;" id="TransferMain">
	...
	<span STYLE="background-color: #CCCCCC" id="LocalCloser"><IMG SRC="../agc/images/vdc_XB_localcloser_OFF.gif" border=0 alt="LOKALNY FINALIZUJĄCYCH"></span> 
	<span STYLE="background-color: #CCCCCC" id="HangupXferLine"><IMG SRC="../agc/images/vdc_XB_hangupxferline_OFF.gif" border=0 alt="Rozłącz linię transferu"></span>
	<span STYLE="background-color: #CCCCCC" id="HangupBothLines"><a href="#" onclick="bothcall_send_hangup();return false;"><IMG SRC="../agc/images/vdc_XB_hangupbothlines.gif" border=0 alt="Rozłącz obie linie"></a></span>

	<BR>

	<IMG SRC="../agc/images/vdc_XB_number.gif" border=0 alt="Numer do wybrania"> <input type=text size=15 name=xfernumber maxlength=25 class="cust_form"> &nbsp; 
	...
	<span STYLE="background-color: #CCCCCC" id="Leave3WayCall"><IMG SRC="../agc/images/vdc_XB_leave3waycall_OFF.gif" border=0 alt="WYJDŹ Z ROZMOWY 3-STRONNEJ"></span> 
	<span STYLE="background-color: #CCCCCC" id="DialBlindTransfer"><IMG SRC="../agc/images/vdc_XB_blindtransfer_OFF.gif" border=0 alt="Wybierz numer do nieweryfikowanego transferu"></span>
	...
	<IMG SRC="../agc/images/vdc_XB_sekundy.gif" border=0 alt="sekundy"> <input type=text size=2 name=xferlength maxlength=4 class="cust_form"> &nbsp; 
	...
	<span STYLE="background-color: #CCCCCC" id="DialWithCustomer"><a href="#" onclick="SendManualDial('YES');return false;"><IMG SRC="../agc/images/vdc_XB_dialwithcustomer.gif" border=0 alt="Połącz z klientem"></a></span> 
	<span STYLE="background-color: #CCCCCC" id="ParkCustomerDial"><a href="#" onclick="xfer_park_dial();return false;"><IMG SRC="../agc/images/vdc_XB_parkcustomerdial.gif" border=0 alt="Zawieś połączenie z klientem"></a></span> 
...
<span style="position:absolute;left:0px;top:12px;z-index:26;" id="NoneInSessionBox">
    <table border=1 bgcolor="#CCFFFF" width=<?=$CAwidth ?> height=500><TR><TD align=center> Nikt nie uczestniczy w Twojej sesji: <span id="NoneInSessionID"></span><BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:27;" id="CustomerGoneBox">
    <table border=1 bgcolor="#CCFFFF" width=<?=$CAwidth ?> height=500><TR><TD align=center> Klient sie rozłączył: <span id="CustomerGoneChanneL"></span><BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:28;" id="WrapupBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=550><TR><TD align=center> Call Wrapup <span id="WrapupTimer"></span> sekundy remaining in wrapup<BR><BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:29;" id="AgenTDisablEBoX">
    <table border=1 bgcolor="#FFFFFF" width=<?=$CAwidth ?> height=500><TR><TD align=center>Wasza sesja była uszkadzana<BR><a href="#" onclick="LogouT();return false;">WYLOGUJ</a><BR><BR><a href="#" onclick="hideDiv('AgenTDisablEBoX');return false;">Wróć</a>
</TD></TR></TABLE>
</span>

<span style="position:absolute;left:0px;top:0px;z-index:30;" id="LogouTBox">
    <table border=1 bgcolor="#FFFFFF" width=<?=$CAwidth ?> height=500><TR><TD align=center><BR><span id="LogouTBoxLink">WYLOGUJ</span></TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:33;" id="DispoButtonHideC">
    <table border=0 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=47><TR><TD align=center VALIGN=top>Zmiany wprowadzone w informacjach o kliencie będą zapisane tylko podczas trwania połączenia. Nie po rozłączaniu!. </TD></TR></TABLE>
</span>

<span style="position:absolute;left:0px;top:0px;z-index:34;" id="DispoSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=460><TR><TD align=center VALIGN=top> REZULTAT ROZMOWY :<span id="DispoSelectPhonE"></span> &nbsp; &nbsp; &nbsp; <span id="DispoSelectHAspan"><a href="#" onclick="DispoHanguPAgaiN()">Rozłącz się ponownie</a></span> &nbsp; &nbsp; &nbsp; <span id="DispoSelectMaxMin"><a href="#" onclick="DispoMinimize()">minimalizuj</a></span><BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:40;" id="PrzerwaCodeSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=460><TR><TD align=center VALIGN=top> WYBÓR SZYFR PRZERWY :<BR>
	<span id="PrzerwaCodeSelectContent"> Przerwa Code Selection </span>
	<input type=hidden name=PrzerwaCodeSelection>
	<BR><BR> &nbsp; 
	</TD></TR></TABLE>
</span>

<span style="position:absolute;left:0px;top:0px;z-index:35;" id="CallBackSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=460><TR><TD align=center VALIGN=top> Wybierz datę oddzwonienia :<span id="CallBackDatE"></span><BR>
	...
	?>
	Notatki: <input type=text name="CallBackCommenTsField" id="CallBackCommenTsField" size=50><BR><BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:36;" id="CloserSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=460><TR><TD align=center VALIGN=top> SELEKCJA GRUPY ZAWNĘTRZNYCH FINALIZUJĄCYCH <BR>
...
<span style="position:absolute;left:154px;top:65px;z-index:17;" id="ScriptPanel">
    <table border=0 bgcolor="#FFE7D0" width=<?=$SSwidth ?> height=<?=$SSheight ?>><TR><TD align=left valign=top><font class="sb_text"><div class="scroll_script" id="ScriptContents">VICIWYBIERZ SCRIPT</div></font></TD></TR></TABLE>
</span>


<? if ( ($HK_statuses_camp > 0) && ( ($user_level>=$HKuser_level) or ($VU_hotkeys_active > 0) ) ) { ?>
<span style="position:absolute;left:<?=$HKwidth ?>px;top:<?=$HKheight ?>px;z-index:16;" id="hotkeysdisplay"><a href="#" onMouseOver="HotKeys('ON')"><IMG SRC="../agc/images/vdc_XB_hotkeysactive_OFF_pl.gif" border=0 alt="KLAWISZE SKRÓTU NIEAKTYWNE"></a></span>
<? } ?>


<span style="position:absolute;left:0px;top:<?=$HKheight ?>px;z-index:15;" id="MaiNfooterspan">
<table BGCOLOR="#E0C2D6" id="MaiNfooter" width=<?=$MNwidth ?>><tr height=32><td height=32><font face="Arial,Helvetica" size=1>wersja klienta VICIWYBIERZ: <? echo $version ?> &nbsp; &nbsp; KOMPILACJA: <? echo $build ?> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Serwer: <? echo $server_ip ?>  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;</font><BR>
<font class="body_small"><span id="busycallsdisplay"><a href="#"  onclick="conf_channels_detail('SHOW');">Pokaż informacje o kanałach konferencyjnych</a><BR><BR>&nbsp;</span></font></td><td align=right height=32>
</td></tr>
...
<span STYLE="background-color: #CCFFCC" id="DiaLControl"><a href="#" onclick="ManualDialNext('','','','','');"><IMG SRC="../agc/images/vdc_LB_dialnextnumber_OFF.gif" border=0 alt="Wybierz kolejny numer"></a></span><BR>
...
<span id="DiaLDiaLAltPhonE"><font class="preview_text"> <input type=checkbox name=DiaLAltPhonE size=1 value="0"> WYBIERANIE DRUGIEGO NUMERU<BR></font></span>
...
if ( ($alt_phone_dialing) and ($auto_dial_level==0) )
	{echo "<font class=\"preview_text\"> <input type=checkbox name=DiaLAltPhonE size=1 value=\"0\"> WYBIERANIE DRUGIEGO NUMERU<BR></font>";}
...
<span STYLE="background-color: #CCCCCC" id="ParkControl"><IMG SRC="../agc/images/vdc_LB_parkcall_OFF_pl.gif" border=0 alt="Zawieś rozmowę"></span><BR>
...
<div class="text_input" id="SendDTMFdiv"><span STYLE="background-color: #CCCCCC" id="SendDTMF"><a href="#" onclick="SendConfDTMF('<?=$session_id ?>');return false;"><IMG SRC="../agc/images/vdc_LB_senddtmf.gif" border=0 alt="Wyślij tonowo" align=bottom></a>  <input type=text size=5 name=conf_dtmf class="cust_form" value="" maxlength=50></div></span><BR>
</center>
</font>
</td>
...
<td align=left><font class="body_text"><input type=text size=3 name=SecondS class="cust_form" value="">&nbsp; Kanał: <input type=text size=6 name=callchannel class="cust_form" value="">&nbsp; Ustawiony Czas: <input type=text size=22 name=custdatetime class="cust_form" value=""></td>
</tr><tr>
...
<td align=left><font class="body_text"><input type=text size=17 name=address2 maxlength=100 class="cust_form" value="">&nbsp; Adres3: <input type=text size=25 name=address3 maxlength=100 class="cust_form" value=""></td>
</tr><tr>
<td align=right><font class="body_text"> Miasto: </td>
<td align=left><font class="body_text"><input type=text size=20 name=city maxlength=50 class="cust_form" value="">&nbsp; Stan: <input type=text size=2 name=state maxlength=2 class="cust_form" value="">&nbsp; KodPocztowy: <input type=text size=9 name=postal_code maxlength=10 class="cust_form" value=""></td>
</tr><tr>
...
<td align=left><font class="body_text"><input type=text size=11 name=phone_number maxlength=12 class="cust_form" value="">&nbsp; Prefiks: <input type=text size=4 name=phone_code maxlength=10 class="cust_form" value="">&nbsp; Drugi telefon: <input type=text size=11 name=alt_phone maxlength=12 class="cust_form" value=""></td>
</tr><tr>
...
<!-- END *********   Here is the main VICIWYBIERZ display panel -->
