...
$SCwidth =  ($MASTERwidth + 230);	# 670 - live call sekundy counter
...
if ($relogin == 'YES')
{
echo "<title>klient VICIDIAL: Powtórne zalogowanie</title>\n";
echo "</head>\n";
...
if ($user_login_first == 1)
{
	...
	{
	echo "<title>klient VICIDIAL: Logowanie do kampani</title>\n";
	echo "</head>\n";
	echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
	echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\">Timeclock</A><BR>\n";
	echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_pl/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Polski <img src=\"../agc/images/pl.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
	...
	else
	{
		...
		echo "<title>klient VICIDIAL: Login</title>\n";
		echo "</head>\n";
		echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
		echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\">Timeclock</A><BR>\n";
		echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
if ( (strlen($phone_login)<2) or (strlen($phone_pass)<2) )
{
echo "<title>klient VICIDIAL:  Login telefonu</title>\n";
echo "</head>\n";
echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\">Timeclock</A><BR>\n";
echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
$VDloginDISPLAY=0;
	...
	if($auth>0)
		...
		if ( (!eregi(" $VD_campaign ",$LOGallowed_campaigns)) and (!eregi("ALL-CAMPAIGNS",$LOGallowed_campaigns)) )
			{
			echo "<title>klient VICIDIAL: VICIDIAL Logowanie do kampani</title>\n";
			echo "</head>\n";
			echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
			echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\">Timeclock</A><BR>\n";
			echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
if (!$authphone)
	{
	echo "<title>klient VICIDIAL: Login telefonu Error</title>\n";
	echo "</head>\n";
	echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
	echo "<A HREF=\"./timeclock.php?referrer=agent&pl=$phone_login&pp=$phone_pass&VD_login=$VD_login&VD_pass=$VD_pass\">Timeclock</A><BR>\n";
	echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
...
### logged in and use that phone login account
	...
	echo "<title>klient VICIDIAL</title>\n";
...
// filter manual dialstring and pass na to originate call
	function SendManualDial(taskFromConf)
		...
		if (taskFromConf == 'YES')
			...
			document.getElementById("DialWithCustomer").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_dialwithcustomer_OFF.gif\" border=0 alt=\"Połącz z klientem\"></a>";

			document.getElementById("ParkCustomerDial").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_parkcustomerdial_OFF.gif\" border=0 alt=\"Zawieś połączenie z klientem\"></a>";
				...
				xmlhttprequestcheckauto.onreadystatechange = function() 
					...
					//	alert(xmlhttprequestcheckauto.responseText);
						...
						//	DialLog("start");
							...
							if (alert_enabled=='ON')
								{
								alert(" Przychodząca: " + status_display_number + "\n Group- " + VDIC_data_VDIG[1] + " &nbsp; " + VDIC_fronter);
								}
							}
						else
							{
	...
	function LogouT(tempreason)
		...
		if (MD_channel_look==1)
			...
			if (VD_live_customer_call==1)
				...
				document.getElementById("LogouTBoxLink").innerHTML = logout_content + "<a href=\"" + agcPAGE + "?relogin=YES&session_epoch=" + epoch_sec + "&session_id=" + session_id + "&session_name=" + session_name + "&VD_login=" + user + "&VD_campaign=" + campaign + "&phone_login=" + phone_login + "&phone_pass=" + phone_pass + "&VD_pass=" + pass + "\">KLINKNIJ TU, ABY PONOWNIE ZALOGOWAĆ SIĘ</a>\n";
...
// GLOBAL FUNCTIONS
	...
	function all_refresh()
		...
		if (vdc_header_date_format.match(regALdate))
			{
			var statusmon='';
			if (mon == 1) {statusmon = "STY";}
			if (mon == 2) {statusmon = "LUT";}
			if (mon == 3) {statusmon = "MAR";}
			if (mon == 4) {statusmon = "KWI";}
			if (mon == 5) {statusmon = "MAJ";}
			if (mon == 6) {statusmon = "CZE";}
			if (mon == 7) {statusmon = "LIP";}
			if (mon == 8) {statusmon = "SIE";}
			if (mon == 9) {statusmon = "WRZ";}
			if (mon == 10) {statusmon = "PAŹ";}
			if (mon == 11) {statusmon = "LIS";}
			if (mon == 12) {statusmon = "GRU";}
...
<?	echo "<a href=\"#\" onclick=\"LogouT('NORMAL');return false;\">WYLOGUJ</a>\n"; ?>
</TD></TR></TABLE>
</SPAN>
...
<span style="position:absolute;left:0px;top:0px;z-index:3;" id="WitajBoxA">
    <table border=0 bgcolor="#FFFFFF" width=<?=$CAwidth ?> height=<?=$HKwidth ?>><TR><TD align=center><BR><span id="WitajBoxAt">VICIDIAL</span></TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:<?=$SCwidth ?>px;top:49px;z-index:18;" id="SecondSspan"><font class="body_text"> sekundy: 
...
<span style="position:absolute;left:5px;top:<?=$HTheight ?>px;z-index:21;" id="TransferMain">
	...
	<font class="body_text">
	 <IMG SRC="../agc/images/vdc_XB_header.gif" border=0 alt="Transfer - Konferencja"> &nbsp; &nbsp; &nbsp; &nbsp; <span id="XfeRDiaLGrouPSelecteD"></span> &nbsp; &nbsp; <span id="XfeRCID"></span><BR>
	...
	<span STYLE="background-color: #CCCCCC" id="Leave3WayCall"><a href="#" onclick="leave_3way_call('FIRST');return false;"><IMG SRC="../agc/images/vdc_XB_leave3waycall.gif" border=0 alt="WYJDŹ Z ROZMOWY 3-STRONNEJ"></a></span> 
...
<span style="position:absolute;left:0px;top:0px;z-index:31;" id="AgenTDisablEBoX">
    <table border=1 bgcolor="#FFFFFF" width=<?=$CAwidth ?> height=500><TR><TD align=center>Wasza sesja była uszkadzana<BR><a href="#" onclick="LogouT('DISABLED');return false;">WYLOGUJ</a><BR><BR><a href="#" onclick="hideDiv('AgenTDisablEBoX');return false;">Wróć</a>
</TD></TR></TABLE>
</span>

<span style="position:absolute;left:0px;top:0px;z-index:32;" id="SysteMDisablEBoX">
    <table border=1 bgcolor="#FFFFFF" width=<?=$CAwidth ?> height=500><TR><TD align=center>Istnieje problem synchronizacji czasu z systemem, należy powiadomić administratora systemu<BR><BR><BR><a href="#" onclick="hideDiv('SysteMDisablEBoX');return false;">Wróć</a>
</TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:44;" id="GroupAliasSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=<?=$CAwidth ?> height=460><TR><TD align=center VALIGN=top> Wybierz grupę Alias :<BR>
...
<table BGCOLOR="<? echo $MAIN_COLOR ?>" id="MaiNfooter" width=<?=$MNwidth ?>><tr height=32><td height=32><font face="Arial,Helvetica" size=1>wersja klienta VICIDIAL: <? echo $version ?> &nbsp; &nbsp; KOMPILACJA: <? echo $build ?> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Serwer: <? echo $server_ip ?>  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;</font><BR>
