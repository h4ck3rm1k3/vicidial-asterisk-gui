...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_el/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Ελληνικά <img src=\"../agc/images/el.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";			echo "</TR></TABLE>\n";
			...
			echo "Εκστρατεία: $camp_form_code<br>\n";
		...
		if($CAMPactive>0)
			...
			if ( (!ereg('DISABLED',$VU_vicidial_recording_override)) and ($VU_vicidial_recording > 0) )
				...
				print "<!-- USER ΕΓΓΡΑΦΗING OVERRIDE: |$VU_vicidial_recording_override|$campaign_recording| -->\n";
				}
			...
			$row=mysql_fetch_row($rslt);
			   ...
			   print "<!-- $campaign_leads_to_call - οδηγοί που απέμειναν για κλήση στον hopper -->\n";
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_el/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Ελληνικά <img src=\"../agc/images/el.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
	...
	if ( (strlen($park_ext)>0) && (strlen($park_file_name)>0) )
		...
		print "<!-- ΠΡΟΣΑΡΜΟΣΜΕΝΟΣ ΧΩΡΟΣ ΣΤΑΘΜΕΥΣΗΣ ΕΚΣΤΡΑΤΕΙΑΣ:  |$VICIDiaL_park_on_extension|$VICIDiaL_park_on_filename| -->\n";
		}
		print "<!-- ΠΡΟΕΠΙΛΟΓΗ ΧΩΡΟΥ ΣΤΑΘΜΕΥΣΗΣ ΕΚΣΤΡΑΤΕΙΑΣ: |$VICIDiaL_park_on_extension|$VICIDiaL_park_on_filename| -->\n";
	...
	if (strlen($web_form_address)>0)
		...
		print "<!-- ΠΡΟΣΑΡΜΟΣΜΕΝΗ ΙΣΤΟΣΕΛΙΔΑ ΕΚΣΤΡΑΤΕΙΑΣ:   |$VICIDiaL_web_form_address| -->\n";
		}
	else
		{
	...
	if ($allow_closers=="Y")
		...
		print "<!-- Η ΕΚΣΤΡΑΤΕΙΑ ΕΠΙΤΡΕΠΕΙ CLOSERS:    |$VICIDiaL_allow_closers| -->\n";
		}
	else
		{
	...
	if ( (eregi("(CLOSER|BLEND|INBND|_C$|_B$|_I$)", $VD_campaign)) || ($campaign_leads_to_call > 0) || (ereg('Y',$no_hopper_leads_logins)) )
		...
		if ($prev_login_ct > 0)
			{print "<!-- ΧΡΗΣΙΜΟΠΟΙΗΣΗ ΠΡΟΗΓΟΥΜΕΝΟΥ ΔΩΜΑΤΙΟΥ MEETME - $session_id - $NOW_TIME - $SIP_user -->\n";}
		else
			{
	...
	#	print "<B>You have logged in as user: $VD_login στο phone: $SIP_user στην εκστρατεία: $VD_campaign</B><BR>\n";
		...
		if ($auto_dial_level > 0)
			{
			print "<!-- η εκστρατεία τίθεται στην αυτόματη κλήση: $auto_dial_level -->\n";
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	alert(xmlhttp.responseText);
					...
					if ( (agentcallsstatus == '1') || (vicidial_agent_disable != 'NOT_ACTIVE') )
						{
						var Astatus_array = check_time_array[2].split("Κατάσταση: ");
						var AGStatuS = Astatus_array[1];
						var CamPCalLs_array = check_time_array[3].split("CampCalls: ");
						var CamPCalLs = CamPCalLs_array[1];
						if (AGStatuS != 'N')
							{
							document.getElementById("AgentΚατάστασηΚατάσταση").innerHTML = AGStatuS;
							}
						if (CamPCalLs != 'N')
							{
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	alert(xmlhttp.responseText);
					...
					if (MDlookCID == "NO")
						{
						MD_ring_secondS++;
						document.getElementById("MainStatuSSpan").innerHTML = " Σε Κλήση: " + lead_dial_number + " UID: " + CIDcheck + " &nbsp; Αναμονή για κουδούνισμα... " + MD_ring_secondS + " δευτερόλεπτα";
				//		alert("channel not found yet:\n" + campaign);
						...
						if (taskCheckOR == 'YES')
							...
							if ( (XDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') && (MD_ring_secondS < 10) )
								...
								document.getElementById("Leave3WayCall").innerHTML ="<a href=\"#\" onclick=\"leave_3way_call('FIRST');return false;\"><IMG SRC=\"../agc/images/vdc_XB_leave3waycall_el.gif\" border=0 alt=\"ΑΠΟΧΩΡΗΣΗ 3μελής ΚΛΗΣΗΣ\"></a>";
								...
								document.getElementById("ParkCustomerDial").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_parkcustomerdial_OFF_el.gif\" border=0 alt=\"Στάθμευση κλήσης πελάτη\">";
							...
							if ( (MDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') )
								...
								MD_channel_look=0;
								document.getElementById("MainStatuSSpan").innerHTML = " Αποκαλούμενος: " + lead_dial_number + " UID: " + CIDcheck + " &nbsp;"; 
								...
								document.getElementById("LocalCloser").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRLOCAL','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_XB_localcloser_el.gif\" border=0 alt=\"ΤΟΠΙΚΟΣ CLOSER\"></a>";

								document.getElementById("InternalCloser").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRINTERNAL','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_XB_internalcloser_el.gif\" border=0 alt=\"ΕΣΩΤΕΡΙΚΟΣ CLOSER\"></a>";

								document.getElementById("DialBlindTransfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRBLIND','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_XB_blindtransfer_el.gif\" border=0 alt=\"Τυφλή μεταφορά κλήσης\"></a>";
				...
				//		alert(xmlhttp.responseText);
					...
					//	DialLog("start");
						...
						if (VDIC_data_VDIG[1].length > 0)
							...
							document.getElementById("MainStatuSSpan").innerHTML = " Εισερχόμενο: " + document.vicidial_form.phone_number.value + " Group- " + VDIC_data_VDIG[1] + " &nbsp; " + VDIC_fronter; 
							}
...
// Send Hangup command for customer call connected to the conference now to Manager
	function dialedcall_send_hangup(dispowindow,hotkeysused,altdispo) 
		...
		//  DEACTIVATE CHANNEL-DEPENDANT BUTTONS AND VARIABLES
			...
			document.getElementById("DialBlindTransfer").innerHTML = "<IMG SRC=\"../agc/images/vdc_XB_blindtransfer_OFF_el.gif\" border=0 alt=\"Τυφλή μεταφορά κλήσης\">";
...
// Send Hangup command for 3rd party call connected to the conference now to Manager
	function xfercall_send_hangup() 
		...
		//  DEACTIVATE CHANNEL-DEPENDANT BUTTONS AND VARIABLES
			...
			document.getElementById("HangupXferLine").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_hangupxferline_OFF_el.gif\" border=0 alt=\"Κλείσιμο Γραμμής Xfer\">";
...
// Generate the Closer In Group Chooser panel
	function CloserSelectContent_create()
		...
		if (VU_agent_choose_ingroups == '1')
			{
			var live_CSC_HTML = "<table cellpadding=5 cellspacing=5 width=500><tr><td><B>ΟΜΑΔΕΣ ΠΟΥ ΔΕΝ ΕΧΟΥΝ ΕΠΙΛΕΓΕΙ</B></td><td><B>ΕΠΙΛΕΓΜΕΝΕΣ ΟΜΑΔΕΣ</B></td></tr><tr><td bgcolor=\"#99FF99\" height=300 width=240 valign=top><font class=\"log_text\"><span id=CloserSelectAdd>";
			var loop_ct = 0;
...
// GLOBAL FUNCTIONS
		...
		//	if ( (agentcall_manual != '1') && (starting_dial_level > 0) )
			...
			if (open_dispo_screen==1)
				...
				if (wrapup_δευτερόλεπτα > 0)	
					{
					showDiv('WrapupBox');
					document.getElementById("WrapupTimer").innerHTML = wrapup_δευτερόλεπτα;
					wrapup_waiting=1;
					}
				CustomerData_update();
			...
			if (WaitingForNextStep==0)
				...
				if (wrapup_δευτερόλεπτα > 0)	
					{
					document.getElementById("WrapupTimer").innerHTML = (wrapup_δευτερόλεπτα - wrapup_counter);
					wrapup_counter++;
					if ( (wrapup_counter > wrapup_δευτερόλεπτα) && (document.getElementById("WrapupBox").style.visibility == 'visible') )
						{
						wrapup_waiting=0;
						hideDiv('WrapupBox');
						if (document.vicidial_form.DispoSelectStop.checked==true)
	...
	function conf_channels_detail(divvar) 
		...
		if (divvar == 'SHOW')
			...
			document.getElementById("busycallsdisplay").innerHTML = "<a href=\"#\"  onclick=\"conf_channels_detail('SHOW');\">Παρουσίαση πληροφοριών κλήσης καναλιού διασκέψης</a>";
			document.getElementById("outboundcallsspan").innerHTML = '';
			LMAe[0]=''; LMAe[1]=''; LMAe[2]=''; LMAe[3]=''; LMAe[4]=''; LMAe[5]=''; 
			LMAcount=0;
			}
...
<span style="position:absolute;left:500px;top:350px;z-index:14;" id="ΠαύσηCodeButtons"><font class="body_text">
...
<span style="position:absolute;left:0px;top:0px;z-index:39;" id="NeWManuaLDiaLBox">
	...
	?>
	Σημείωση: όλες οι νέες χειροκίνητες κλήσεις θα πάνε στην λίστα 999<BR><BR>
	<table><tr>
...
<span style="position:absolute;left:5px;top:310px;z-index:21;" id="TransferMain">
	...
	<span STYLE="background-color: #CCCCCC" id="InternalCloser"><IMG SRC="../agc/images/vdc_XB_internalcloser_el.gif" border=0 alt="ΕΣΩΤΕΡΙΚΟΣ CLOSER"></span>
	...
	<span STYLE="background-color: #CCCCCC" id="HangupXferLine"><IMG SRC="../agc/images/vdc_XB_hangupxferline_OFF_el.gif" border=0 alt="Κλείσιμο Γραμμής Xfer"></span>
	<span STYLE="background-color: #CCCCCC" id="HangupBothLines"><a href="#" onclick="bothcall_send_hangup();return false;"><IMG SRC="../agc/images/vdc_XB_hangupbothlines_el.gif" border=0 alt="Κλείσιμο και τις δύο γραμμές"></a></span>
	...
	<IMG SRC="../agc/images/vdc_XB_δευτερόλεπτα_el.gif" border=0 alt="δευτερόλεπτα"> <input type=text size=2 name=xferlength maxlength=4 class="cust_form"> &nbsp; 
	<IMG SRC="../agc/images/vdc_XB_channel_el.gif" border=0 alt="channel"> <input type=text size=12 name=xferchannel maxlength=100 class="cust_form"> 
...
<span style="position:absolute;left:0px;top:0px;z-index:29;" id="AgenTDisablEBoX">
    <table border=1 bgcolor="#FFFFFF" width=770 height=500><TR><TD align=center>Η σύνοδός σας έχει τεθεί εκτός λειτουργίας<BR><a href="#" onclick="LogouT();return false;">ΑΠΟΣΥΝΔΕΣΗ</a><BR><BR><a href="#" onclick="hideDiv('AgenTDisablEBoX');return false;">Επιστροφή</a>
</TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:40;" id="ΠαύσηCodeSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=770 height=460><TR><TD align=center VALIGN=top> ΕΠΙΛΕΞΤΕ έναν ΚΩΔΙΚΑ ΜΙΚΡΗΣ ΔΙΑΚΟΠΉΣ :<BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:35;" id="CallBackSelectBox">
	...
	?>
	Σχόλια CB: <input type=text name="CallBackCommenTsField" size=50><BR><BR>
...
<tr><td align=left colspan=3>
<table><tr height=32><td height=32><font face="Arial,Helvetica" size=1>Έκδοση VICIΚΛΗΣΗ: <? echo $version ?> &nbsp; &nbsp; ΔΗΜΙΟΥΡΓΙΑ: <? echo $build ?> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Κεντρικός υπολογιστής: <? echo $server_ip ?>  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;</font><BR>
<font class="body_small"><span id="busycallsdisplay"><a href="#"  onclick="conf_channels_detail('SHOW');">Παρουσίαση πληροφοριών κλήσης καναλιού διασκέψης</a></span></font></td><td align=right height=32>
<? if ( ($HK_statuses_camp > 0) && ( ($user_level>=$HKuser_level) or ($VU_hotkeys_active > 0) ) ) { ?>
<span id="hotkeysdisplay"><a href="#" onMouseOver="HotKeys('ON')"><IMG SRC="../agc/images/vdc_XB_hotkeysactive_OFF_el.gif" border=0 alt="ΠΛΗΚΤΡΑ ΚΛΕΙΔΙΑ ΑΝΕΝΕΡΓΑ"></a></span>
<? } ?>
</td></tr>
