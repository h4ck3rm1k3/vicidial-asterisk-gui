...
#	Header("HTTP/1.0 401 Unauthorized");
    echo "Nombre y contraseña inválidos del usuario or no export report permission: |$PHP_AUTH_USER|\n";
    exit;
	}
...
if ($run_export > 0)
	...
	if ( ($outbound_to_print > 0) or ($inbound_to_print > 0) )
		...
		$stmt="INSERT INTO vicidial_admin_log set event_date='$NOW_TIME', user='$PHP_AUTH_USER', ip_address='$ip', event_section='LEADS', event_type='EXPORT', record_id='', event_code='ADMIN PIDE INFORME DE EXPORTACIÓN', event_sql=\"$SQL_log\", event_notes='';";
		if ($DB) {echo "|$stmt|\n";}
		$rslt=mysql_query($stmt, $link);
...
##### END RUN THE EXPORT AND OUTPUT FLAT DATA FILE #####
	...
	echo "<TITLE>VICIDIAL: Pide Informe exportación";
