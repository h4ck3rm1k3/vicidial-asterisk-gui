...
if ($relogin == 'YES')
{
echo "<title>Web client de VICISELETOR: Re-Início de uma sessão</title>\n";
echo "</head>\n";
...
if ($user_login_first == 1)
{
	...
	{
	echo "<title>Web client de VICISELETOR: Início de uma sessão Da Campanha</title>\n";
	echo "</head>\n";
	echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
	echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
	echo "<!-- ILPV -->\n";
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_pt/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Português <img src=\"../agc/images/pt.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
	...
	else
	{
		...
		echo "<title>Web client de VICISELETOR: Início de uma sessão</title>\n";
		echo "</head>\n";
		echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
		echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
		echo "<!-- ILPV -->\n";
...
if ( (strlen($phone_login)<2) or (strlen($phone_pass)<2) )
{
echo "<title>Web client de VICISELETOR:  Início de uma sessão Do Telefone</title>\n";
echo "</head>\n";
echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
echo "<!-- ILPV -->\n";
	...
	if($auth>0)
		...
		if($CAMPactive>0)
			...
			$row=mysql_fetch_row($rslt);
			   ...
			   print "<!-- $campaign_leads_to_call - conduz à esquerda para chamar-se no funil -->\n";
...
if (!$authphone)
	{
	echo "<title>Web client de VICISELETOR: Início de uma sessão Do Telefone Error</title>\n";
	echo "</head>\n";
	echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0>\n";
	echo "<TABLE WIDTH=100%><TR><TD></TD>\n";
	echo "<!-- ILPV -->\n";
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_pt/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">Português <img src=\"../agc/images/pt.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";	echo "</TR></TABLE>\n";
	...
	{
	echo "<title>Web client de VICISELETOR</title>\n";
	...
	if ( (strlen($park_ext)>0) && (strlen($park_file_name)>0) )
		...
		print "<!-- ESTACIONAMENTO FEITO SOB ENCOMENDA DA CAMPANHA:  |$VICIDiaL_park_on_extension|$VICIDiaL_park_on_filename| -->\n";
		}
		print "<!-- ESTACIONAMENTO DO DEFEITO DA CAMPANHA: |$VICIDiaL_park_on_extension|$VICIDiaL_park_on_filename| -->\n";
	...
	if (strlen($web_form_address)>0)
		...
		print "<!-- FORMULÁRIO FEITO SOB ENCOMENDA DA CORREIA FOTORRECEPTORA DA CAMPANHA:   |$VICIDiaL_web_form_address| -->\n";
		}
	else
		{
	...
	if ($allow_closers=="Y")
		...
		print "<!-- A CAMPANHA PERMITE CLOSERS:    |$VICIDiaL_allow_closers| -->\n";
		}
	else
		{
		$VICIDiaL_allow_closers = 0;
		print "<!-- A CAMPANHA NÃO PERMITE NENHUM CLOSERS: |$VICIDiaL_allow_closers| -->\n";
		}
	...
	if ( (eregi("CLOSER", $VD_campaign)) || ($campaign_leads_to_call > 0) )
		...
		if ($prev_login_ct > 0)
			{print "<!-- USANDO O QUARTO PRECEDENTE DE MEETME - $session_id - $NOW_TIME - $SIP_user -->\n";}
		else
			{
			...
			$rslt=mysql_query($stmt, $link);
			print "<!-- USANDO O QUARTO NOVO DE MEETME - $session_id - $NOW_TIME - $SIP_user -->\n";
...
?>
	...
	var DiaLControl_auto_HTML = "<IMG SRC=\"../agc/images/vdc_LB_pause_OFF_pt.gif\" border=0 alt=\"Pausa\"><a href=\"#\" onclick=\"AutoDial_ReSume_PauSe('VDADready');\"><IMG SRC=\"../agc/images/vdc_LB_resume_pt.gif\" border=0 alt=\"Resumo\"></a>";
	var DiaLControl_auto_HTML_ready = "<a href=\"#\" onclick=\"AutoDial_ReSume_PauSe('VDADpause');\"><IMG SRC=\"../agc/images/vdc_LB_pause_pt.gif\" border=0 alt=\"Pausa\"></a><IMG SRC=\"../agc/images/vdc_LB_resume_OFF_pt.gif\" border=0 alt=\"Resumo\">";
	var DiaLControl_auto_HTML_OFF = "<IMG SRC=\"../agc/images/vdc_LB_pause_OFF_pt.gif\" border=0 alt=\"Pausa\"><IMG SRC=\"../agc/images/vdc_LB_resume_OFF_pt.gif\" border=0 alt=\"Resumo\">";
	var DiaLControl_manual_HTML = "<a href=\"#\" onclick=\"ManualDialNext('','','','','');\"><IMG SRC=\"../agc/images/vdc_LB_dialnextnumber_pt.gif\" border=0 alt=\"Número Seguinte Do Seletor\"></a>";
	...
	var image_LB_dialnextnumber = new Image();
		image_LB_dialnextnumber.src="../agc/images/vdc_LB_dialnextnumber_pt.gif";
	var image_LB_hangupcustomer = new Image();
		image_LB_hangupcustomer.src="../agc/images/vdc_LB_hangupcustomer_pt.gif";
	var image_LB_transferconf = new Image();
		image_LB_transferconf.src="../agc/images/vdc_LB_transferconf_pt.gif";
	var image_LB_grabparkedcall = new Image();
		image_LB_grabparkedcall.src="../agc/images/vdc_LB_grabparkedcall_pt.gif";
	var image_LB_parkcall = new Image();
		image_LB_parkcall.src="../agc/images/vdc_LB_parkcall_pt.gif";
	var image_LB_webform = new Image();
		image_LB_webform.src="../agc/images/vdc_LB_webform_pt.gif";
	var image_LB_stoprecording = new Image();
		image_LB_stoprecording.src="../agc/images/vdc_LB_stoprecording_pt.gif";
	var image_LB_startrecording = new Image();
		image_LB_startrecording.src="../agc/images/vdc_LB_startrecording_pt.gif";
	var image_LB_pause = new Image();
		image_LB_pause.src="../agc/images/vdc_LB_pause_pt.gif";
	var image_LB_resume = new Image();
		image_LB_resume.src="../agc/images/vdc_LB_resume_pt.gif";
	var image_LB_senddtmf = new Image();
		image_LB_senddtmf.src="../agc/images/vdc_LB_senddtmf_pt.gif";
	var image_LB_dialnextnumber_OFF = new Image();
		image_LB_dialnextnumber_OFF.src="../agc/images/vdc_LB_dialnextnumber_OFF_pt.gif";
	var image_LB_hangupcustomer_OFF = new Image();
		image_LB_hangupcustomer_OFF.src="../agc/images/vdc_LB_hangupcustomer_OFF_pt.gif";
	var image_LB_transferconf_OFF = new Image();
		image_LB_transferconf_OFF.src="../agc/images/vdc_LB_transferconf_OFF_pt.gif";
	...
	var image_LB_parkcall_OFF = new Image();
		image_LB_parkcall_OFF.src="../agc/images/vdc_LB_parkcall_OFF_pt.gif";
	var image_LB_webform_OFF = new Image();
		image_LB_webform_OFF.src="../agc/images/vdc_LB_webform_OFF_pt.gif";
	...
	var image_LB_pause_OFF = new Image();
		image_LB_pause_OFF.src="../agc/images/vdc_LB_pause_OFF_pt.gif";
	var image_LB_resume_OFF = new Image();
		image_LB_resume_OFF.src="../agc/images/vdc_LB_resume_OFF_pt.gif";
	var image_LB_senddtmf_OFF = new Image();
		image_LB_senddtmf_OFF.src="../agc/images/vdc_LB_senddtmf_OFF_pt.gif";
	...
	function basic_originate_call(tasknum,taskprefix,taskreverse,taskdialvalue,tasknowait) 
		...
		if (xmlhttp) 
			...
			xmlhttp.onreadystatechange = function() 
				...
				//		document.getElementById("HangupXferLine").innerHTML ="<a href=\"#\" onclick=\"xfercall_send_hangup();return false;\"><IMG SRC=\"../agc/images/vdc_XB_hangupxferline_pt.gif\" border=0 alt=\"Linha De Xfer Do Hangup\"></a>";
						}
					}
				}
			delete xmlhttp;
		...
		if (xmlhttp) 
			...
			xmlhttp.onreadystatechange = function() 
				...
				//			var live_conf_HTML = "<font face=\"Arial,Helvetica\"><B>VIVEM AS CHAMADAS NCESTA CONFERÊNCIA:</B></font><BR><TABLE WIDTH=500><TR BGCOLOR=#E6E6E6><TD><font class=\"log_title\">#</TD><TD><font class=\"log_title\">CANALETA REMOTA</TD><TD><font class=\"log_title\">HANGUP</TD><TD><font class=\"log_title\">XFER</TD></TR>";
							var live_conf_HTML = "<font face=\"Arial,Helvetica\"><B>VIVEM AS CHAMADAS EM SUA SESSÃO:</B></font><BR><TABLE WIDTH=500><TR BGCOLOR=#E6E6E6><TD><font class=\"log_title\">#</TD><TD><font class=\"log_title\">CANALETA REMOTA</TD><TD><font class=\"log_title\">HANGUP</TD></TR>";
	...
	function conf_send_recording(taskconfrectype,taskconfrec,taskconffile) 
		...
		if (xmlhttp) 
			...
			//	filename = filedate + "_" + user_abb;
				...
				var conf_rec_start_html = "<a href=\"#\" onclick=\"conf_send_recording('StopMonitorConf','" + taskconfrec + "','" + filename + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_stoprecording_pt.gif\" border=0 alt=\"Pare de Gravar\"></a>";
			...
			if (taskconfrectype == 'StopMonitorConf')
				...
				var channelrec = "Local/" + conf_silent_prefix + '' + taskconfrec + "@" + ext_context;
				var conf_rec_start_html = "<a href=\"#\" onclick=\"conf_send_recording('MonitorConf','" + taskconfrec + "','');return false;\"><IMG SRC=\"../agc/images/vdc_LB_startrecording_pt.gif\" border=0 alt=\"Comece Gravar\"></a>";
				document.getElementById("RecorDControl").innerHTML = conf_rec_start_html;
				}
	...
	function mainxfer_send_redirect(taskvar,taskxferconf,taskserverip) 
		...
		if (xmlhttp) 
			...
			if (taskvar == 'ParK')
				...
				document.getElementById("ParkControl").innerHTML ="<a href=\"#\" onclick=\"mainxfer_send_redirect('FROMParK','" + redirectdestination + "','" + redirectdestserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_grabparkedcall_pt.gif\" border=0 alt=\"Chamada Estacionada Garra\"></a>";
				customerparked=1;
				}
			if (taskvar == 'FROMParK')
				{
				...
				document.getElementById("ParkControl").innerHTML ="<a href=\"#\" onclick=\"mainxfer_send_redirect('ParK','" + redirectdestination + "','" + redirectdestserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_parkcall_pt.gif\" border=0 alt=\"Chamada Do Parque\"></a>";
				customerparked=0;
				}
			...
			xmlhttp.onreadystatechange = function() 
				...
				//		alert("VICISELETOR Call log entered:\n" + document.vicidial_form.uniqueid.value);
						if ( (taskMDstage != "start") && (VDstop_rec_after_each_call == 1) )
							{
							var conf_rec_start_html = "<a href=\"#\" onclick=\"conf_send_recording('MonitorConf','" + session_id + "','');return false;\"><IMG SRC=\"../agc/images/vdc_LB_startrecording_pt.gif\" border=0 alt=\"Comece Gravar\"></a>";
...
// Request list of USERONLY callbacks for this agent
	function CalLBacKsLisTCheck()
		...
		if ( (AutoDialWaiting == 1) || (VD_live_customer_call==1) || (alt_dial_active==1) )
			{
			alert("VOCÊ DEVE SER PAUSADO PARA VERIFICAR RECHAMADAS NA MODALIDADE DE AUTO-SELETOR");
			}
		else
			{
...
// Open page to enter details for a new manual dial lead
	function NeWManuaLDiaLCalL()
		...
		if ( (AutoDialWaiting == 1) || (VD_live_customer_call==1) || (alt_dial_active==1) )
			{
			alert("VOCÊ DEVE SER PAUSADO Ao SELETOR MANUAL Uma LIGAÇÃO NOVA Na MODALIDADE De AUTO-SELETOR");
			}
		else
			{
				...
				//		alert("channel not found yet:\n" + campaign);
						...
						if (taskCheckOR == 'YES')
							...
							if ( (XDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') )
								...
								document.getElementById("Leave3WayCall").innerHTML ="<a href=\"#\" onclick=\"leave_3way_call();return false;\"><IMG SRC=\"../agc/images/vdc_XB_leave3waycall_pt.gif\" border=0 alt=\"CHAMADA DA LICENÇA 3-WAY\"></a>";

								document.getElementById("DialWithCustomer").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_dialwithcustomer_OFF_pt.gif\" border=0 alt=\"Seletor Com Cliente\">";

								document.getElementById("ParkCustomerDial").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_parkcustomerdial_OFF_pt.gif\" border=0 alt=\"Seletor Do Cliente Do Parque\">";

								document.getElementById("HangupXferLine").innerHTML ="<a href=\"#\" onclick=\"xfercall_send_hangup();return false;\"><IMG SRC=\"../agc/images/vdc_XB_hangupxferline_pt.gif\" border=0 alt=\"Linha De Xfer Do Hangup\"></a>";

								document.getElementById("HangupBothLines").innerHTML ="<a href=\"#\" onclick=\"bothcall_send_hangup();return false;\"><IMG SRC=\"../agc/images/vdc_XB_hangupbothlines_pt.gif\" border=0 alt=\"Hangup Ambas as Linhas\"></a>";
							...
							if ( (MDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') )
								...
								document.getElementById("ParkControl").innerHTML ="<a href=\"#\" onclick=\"mainxfer_send_redirect('ParK','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_parkcall_pt.gif\" border=0 alt=\"Chamada Do Parque\"></a>";
								...
								document.getElementById("XferControl").innerHTML = "<a href=\"#\" onclick=\"ShoWTransferMain('ON');\"><IMG SRC=\"../agc/images/vdc_LB_transferconf_pt.gif\" border=0 alt=\"Transferência - Conferência\"></a>";

								document.getElementById("LocalCloser").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRLOCAL','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_XB_localcloser_pt.gif\" border=0 alt=\"LOCAL MAIS PRÓXIMO\"></a>";

								document.getElementById("InternalCloser").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRINTERNAL','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_XB_internalcloser_pt.gif\" border=0 alt=\"MAIS PRÓXIMO INTERNO\"></a>";

								document.getElementById("DialBlindTransfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRBLIND','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_XB_blindtransfer_pt.gif\" border=0 alt=\"Transferência Da Cortina Do Seletor\"></a>";
...
// Send the Manual Número Seguinte Do Seletor request
	function ManualDialNext(mdnCBid,mdnBDleadid,mdnDiaLCodE,mdnPhonENumbeR,mdnStagE)
		{
		all_record = 'NO';
		document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_dialnextnumber_OFF_pt.gif\" border=0 alt=\"Número Seguinte Do Seletor\">";
...
// $VICISELETOR_web_QUERY_STRING =~ s/\`|\~|\:|\;|\#|\'|\"|\{|\}|\(|\)|\*|\^|\%|\$|\!|\%|\r|\t|\n//gi;
				...
				xmlhttp.onreadystatechange = function() 
					...
					//	alert(xmlhttp.responseText);
						...
						if (MDSnextCID == "LEAD NOT REVERTED")
							...
							document.getElementById("DiaLControl").innerHTML = "<a href=\"#\" onclick=\"ManualDialNext('','','','','');\"><IMG SRC=\"../agc/images/vdc_LB_dialnextnumber_pt.gif\" border=0 alt=\"Número Seguinte Do Seletor\"></a>";
							}
						}
					}
				delete xmlhttp;
...
// refresh the content of the web form URL
	function WebFormRefresH(taskrefresh) 
		...
		if (taskrefresh == 'OUT')
			{
			document.getElementById("WebFormSpan").innerHTML = "<a href=\"" + VDIC_web_form_address + web_form_vars + "\" target=\"vdcwebform\" onMouseOver=\"WebFormRefresH('IN');\"><IMG SRC=\"../agc/images/vdc_LB_webform_pt.gif\" border=0 alt=\"Formulário Da Correia fotorreceptora\"></a>\n";
			}
		else 
			{
			document.getElementById("WebFormSpan").innerHTML = "<a href=\"" + VDIC_web_form_address + web_form_vars + "\" target=\"vdcwebform\" onMouseOut=\"WebFormRefresH('OUT');\"><IMG SRC=\"../agc/images/vdc_LB_webform_pt.gif\" border=0 alt=\"Formulário Da Correia fotorreceptora\"></a>\n";
			}
		}
...
// Send Hangup command for customer call connected to the conference now to Manager
	function dialedcall_send_hangup(dispowindow) 
		...
		//	UPDATE VICISELETOR_LOG ENTRY FOR THIS CHAMADA PROCESS
			DialLog("end");
			if (dispowindow == 'NO')
				{
				open_dispo_screen=0;
		...
		//  DEACTIVATE CHANNEL-DEPENDANT BUTTONS AND VARIABLES
			...
			if( document.images ) { document.images['livecall'].src = image_livecall_OFF.src;}
			document.getElementById("WebFormSpan").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_webform_OFF_pt.gif\" border=0 alt=\"Formulário Da Correia fotorreceptora\">";
			document.getElementById("ParkControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_parkcall_OFF_pt.gif\" border=0 alt=\"Chamada Do Parque\">";
			document.getElementById("HangupControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_hangupcustomer_OFF_pt.gif\" border=0 alt=\"Cliente Do Hangup\">";
			document.getElementById("XferControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_transferconf_OFF_pt.gif\" border=0 alt=\"Transferência - Conferência\">";
			document.getElementById("LocalCloser").innerHTML = "<IMG SRC=\"../agc/images/vdc_XB_localcloser_OFF_pt.gif\" border=0 alt=\"LOCAL MAIS PRÓXIMO\">";
			document.getElementById("InternalCloser").innerHTML = "<IMG SRC=\"../agc/images/vdc_XB_internalcloser_OFF_pt.gif\" border=0 alt=\"MAIS PRÓXIMO INTERNO\">";
			document.getElementById("DialBlindTransfer").innerHTML = "<IMG SRC=\"../agc/images/vdc_XB_blindtransfer_OFF_pt.gif\" border=0 alt=\"Transferência Da Cortina Do Seletor\">";
...
// Send Hangup command for 3rd party call connected to the conference now to Manager
	function xfercall_send_hangup() 
		...
		//  DEACTIVATE CHANNEL-DEPENDANT BUTTONS AND VARIABLES
			...
			document.getElementById("Leave3WayCall").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_leave3waycall_OFF_pt.gif\" border=0 alt=\"CHAMADA DA LICENÇA 3-WAY\">";

			document.getElementById("DialWithCustomer").innerHTML ="<a href=\"#\" onclick=\"SendManualDial('YES');return false;\"><IMG SRC=\"../agc/images/vdc_XB_dialwithcustomer_pt.gif\" border=0 alt=\"Seletor Com Cliente\"></a>";

			document.getElementById("ParkCustomerDial").innerHTML ="<a href=\"#\" onclick=\"xfer_park_dial();return false;\"><IMG SRC=\"../agc/images/vdc_XB_parkcustomerdial_pt.gif\" border=0 alt=\"Seletor Do Cliente Do Parque\"></a>";

			document.getElementById("HangupXferLine").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_hangupxferline_OFF_pt.gif\" border=0 alt=\"Linha De Xfer Do Hangup\">";
...
// ################################################################################
// Mostra message that there are no voice channels in the VICISELETOR session
...
// Generate the Closer In Group Chooser panel
	function CloserSelectContent_create()
		...
		if (VU_agent_choose_ingroups == '1')
			{
			var live_CSC_HTML = "<table cellpadding=5 cellspacing=5 width=500><tr><td><B>GRUPOS NÃO SELECIONADOS</B></td><td><B>GRUPOS SELECIONADOS</B></td></tr><tr><td bgcolor=\"#99FF99\" height=300 width=240 valign=top><font class=\"log_text\"><span id=CloserSelectAdd>";
			var loop_ct = 0;
	...
	function LogouT()
		...
		if (MD_channel_look==1)
			...
			if (VD_live_customer_call==1)
				...
				document.getElementById("LogouTBoxLink").innerHTML = "<a href=\"" + agcPAGE + "?relogin=YES&session_epoch=" + epoch_sec + "&session_id=" + session_id + "&session_name=" + session_name + "&VD_login=" + user + "&VD_campaign=" + campaign + "&phone_login=" + phone_login + "&phone_pass=" + phone_pass + "&VD_pass=" + pass + "\">ESTALE AQUI PARA LOGON OUTRA VEZ</a>\n";
...
// Submitting the callback date and time to the system
	function CallBackDatE_submit()
		...
		//	alert(CallBackDatETimE + "|" + CallBackCommenTs);
			...
			document.vicidial_form.DispoSelection.value = 'CBHVELHO';
...
// GLOBAL FUNCTIONS
	...
	function start_all_refresh()
		...
		//	if ( (agentcall_manual != '1') && (starting_dial_level > 0) )
			...
			if (wrapup_segundos > 0)	
				{
				document.getElementById("WrapupTimer").innerHTML = (wrapup_segundos - wrapup_counter);
				wrapup_counter++;
				if ( (wrapup_counter > wrapup_segundos) && (document.getElementById("WrapupBox").style.visibility == 'visible') )
					{
					wrapup_waiting=0;
					hideDiv('WrapupBox');
					if (document.vicidial_form.DispoSelectStop.checked==true)
	...
	function HotKeys(HKstate) 
		...
		if ( (HKstate == 'ON') && (HKbutton_allowed == 1) )
			...
			showDiv('HotKeyEntriesBox');
			hot_keys_active = 1;
			document.getElementById("hotkeysdisplay").innerHTML = "<a href=\"#\" onMouseOut=\"HotKeys('OFF')\"><IMG SRC=\"../agc/images/vdc_XB_hotkeysactive_pt.gif\" border=0 alt=\"CHAVES QUENTES ATIVAS\"></a>";
			}
		else
			{
			hideDiv('HotKeyEntriesBox');
			hot_keys_active = 0;
			document.getElementById("hotkeysdisplay").innerHTML = "<a href=\"#\" onMouseOver=\"HotKeys('ON')\"><IMG SRC=\"../agc/images/vdc_XB_hotkeysactive_OFF_pt.gif\" border=0 alt=\"CHAVES QUENTES INATIVAS\"></a>";
			}
		}
	...
	function ShoWTransferMain(showxfervar,showoffvar)
		...
		if (VU_vicidial_transfers == '1')
			...
			if (showxfervar == 'ON')
				...
				document.getElementById("XferControl").innerHTML = "<a href=\"#\" onclick=\"ShoWTransferMain('OFF','YES');\"><IMG SRC=\"../agc/images/vdc_LB_transferconf_pt.gif\" border=0 alt=\"Transferência - Conferência\"></a>";
				}
			else
				{
				HKbutton_allowed = 1;
...
<?	echo "Usuário: $VD_login em Telefone: $SIP_user à campanha: $VD_campaign&nbsp; \n"; ?>
...
<TD WIDTH=109><IMG SRC="../agc/images/agc_live_call_OFF_pt.gif" NAME=livecall ALT="Vive A Chamada" WIDTH=109 HEIGHT=30 Border=0></TD>
</TR></TABLE>
</span>
...
<span id="MDstatusSpan"><a href="#" onclick="NeWManuaLDiaLCalL();return false;">SELETOR MANUAL</a></span> <BR>
</font></span>
...
<span style="position:absolute;left:0px;top:0px;z-index:36;" id="NeWManuaLDiaLBox">
	...
	<td align=left><font class="body_text"><input type=text size=12 maxlength=10 name=MDPhonENumbeR class="cust_form" value="">&nbsp; (máximo de 10 dígitos - dígitos somente)</td>
	</tr><tr>
...
<span style="position:absolute;left:5px;top:310px;z-index:21;" id="TransferMain">
	...
	<td align=left>
	<font class="body_text">
	 <IMG SRC="../agc/images/vdc_XB_header_pt.gif" border=0 alt="Transferência - Conferência"><BR>
	<span STYLE="background-color: #CCCCCC" id="InternalCloser"><IMG SRC="../agc/images/vdc_XB_internalcloser_pt.gif" border=0 alt="MAIS PRÓXIMO INTERNO"></span>
	<span STYLE="background-color: #CCCCCC" id="LocalCloser"><IMG SRC="../agc/images/vdc_XB_localcloser_OFF_pt.gif" border=0 alt="LOCAL MAIS PRÓXIMO"></span> 
	...
	<span STYLE="background-color: #CCCCCC" id="HangupXferLine"><IMG SRC="../agc/images/vdc_XB_hangupxferline_OFF_pt.gif" border=0 alt="Linha De Xfer Do Hangup"></span>
	<span STYLE="background-color: #CCCCCC" id="HangupBothLines"><a href="#" onclick="bothcall_send_hangup();return false;"><IMG SRC="../agc/images/vdc_XB_hangupbothlines_pt.gif" border=0 alt="Hangup Ambas as Linhas"></a></span>
	
	<BR>

	<IMG SRC="../agc/images/vdc_XB_number_pt.gif" border=0 alt="Número a chamar-se"> <input type=text size=15 name=xfernumber maxlength=25 class="cust_form"> &nbsp; 
	<IMG SRC="../agc/images/vdc_XB_segundos_pt.gif" border=0 alt="segundos"> <input type=text size=2 name=xferlength maxlength=4 class="cust_form"> &nbsp; 
	<IMG SRC="../agc/images/vdc_XB_channel_pt.gif" border=0 alt="channel"> <input type=text size=12 name=xferchannel maxlength=100 class="cust_form"> 
	...
	<span STYLE="background-color: #CCCCCC" id="DialWithCustomer"><a href="#" onclick="SendManualDial('YES');return false;"><IMG SRC="../agc/images/vdc_XB_dialwithcustomer_pt.gif" border=0 alt="Seletor Com Cliente"></a></span> 
	<span STYLE="background-color: #CCCCCC" id="ParkCustomerDial"><a href="#" onclick="xfer_park_dial();return false;"><IMG SRC="../agc/images/vdc_XB_parkcustomerdial_pt.gif" border=0 alt="Seletor Do Cliente Do Parque"></a></span> 
	<span STYLE="background-color: #CCCCCC" id="Leave3WayCall"><IMG SRC="../agc/images/vdc_XB_leave3waycall_OFF_pt.gif" border=0 alt="CHAMADA DA LICENÇA 3-WAY"></span> 
	<span STYLE="background-color: #CCCCCC" id="DialBlindTransfer"><IMG SRC="../agc/images/vdc_XB_blindtransfer_OFF_pt.gif" border=0 alt="Transferência Da Cortina Do Seletor"></span>
...
<span STYLE="background-color: #CCFFCC" id="DiaLControl"><a href="#" onclick="ManualDialNext('','','','','');"><IMG SRC="../agc/images/vdc_LB_dialnextnumber_OFF_pt.gif" border=0 alt="Número Seguinte Do Seletor"></a></span><BR>
...
<span STYLE="background-color: #CCCCCC" id="RecorDControl"><a href="#" onclick="conf_send_recording('MonitorConf','<?=$session_id ?>','');return false;"><IMG SRC="../agc/images/vdc_LB_startrecording_pt.gif" border=0 alt="Comece Gravar"></a></span><BR>
<span id="SpacerSpanA"><IMG SRC="../agc/images/blank.gif" width=145 height=16 border=0></span><BR>
<span STYLE="background-color: #FFFFFF" id="WebFormSpan"><IMG SRC="../agc/images/vdc_LB_webform_OFF_pt.gif" border=0 alt="Formulário Da Correia fotorreceptora"></span><BR>
<span id="SpacerSpanB"><IMG SRC="../agc/images/blank.gif" width=145 height=16 border=0></span><BR>
<span STYLE="background-color: #CCCCCC" id="ParkControl"><IMG SRC="../agc/images/vdc_LB_parkcall_OFF_pt.gif" border=0 alt="Chamada Do Parque"></span><BR>
<span STYLE="background-color: #CCCCCC" id="XferControl"><IMG SRC="../agc/images/vdc_LB_transferconf_OFF_pt.gif" border=0 alt="Transferência - Conferência"></span><BR>
<span id="SpacerSpanC"><IMG SRC="../agc/images/blank.gif" width=145 height=16 border=0></span><BR>
<span STYLE="background-color: #FFCCFF" id="HangupControl"><IMG SRC="../agc/images/vdc_LB_hangupcustomer_OFF_pt.gif" border=0 alt="Cliente Do Hangup"></span><BR>
<span id="SpacerSpanD"><IMG SRC="../agc/images/blank.gif" width=145 height=16 border=0></span><BR>
<span STYLE="background-color: #CCCCCC" id="SendDTMF"><a href="#" onclick="SendConfDTMF('<?=$session_id ?>');return false;"><IMG SRC="../agc/images/vdc_LB_senddtmf_pt.gif" border=0 alt="Emita DTMF"></a>  <input type=text size=5 name=conf_dtmf class="cust_form" value=""></span><BR>
</center>
</font>
</td>
...
<table><tr>
<td align=right><font class="body_text"> segundos: </td>
<td align=left><font class="body_text"><input type=text size=3 name=SecondS class="cust_form" value="">&nbsp; Canaleta: <input type=text size=6 name=callchannel class="cust_form" value="">&nbsp; Tempo De Cust: <input type=text size=22 name=custdatetime class="cust_form" value=""></td>
</tr><tr>
...
<td align=left><font class="body_text"><input type=text size=17 name=address2 maxlength=100 class="cust_form" value="">&nbsp; Endereço3: <input type=text size=25 name=address3 maxlength=100 class="cust_form" value=""></td>
</tr><tr>
<td align=right><font class="body_text"> Cidade: </td>
<td align=left><font class="body_text"><input type=text size=20 name=city maxlength=50 class="cust_form" value="">&nbsp; Estado: <input type=text size=2 name=state maxlength=2 class="cust_form" value="">&nbsp; PostCode: <input type=text size=9 name=postal_code maxlength=10 class="cust_form" value=""></td>
</tr><tr>
<td align=right><font class="body_text"> Província: </td>
<td align=left><font class="body_text"><input type=text size=20 name=province maxlength=50 class="cust_form" value="">&nbsp; Vendedor Id: <input type=text size=15 name=vendor_lead_code maxlength=20 class="cust_form" value=""></td>
</tr><tr>
<td align=right><font class="body_text"> Telefone: </td>
<td align=left><font class="body_text"><input type=text size=11 name=phone_number maxlength=10 class="cust_form" value="">&nbsp; DialCode: <input type=text size=4 name=phone_code maxlength=10 class="cust_form" value="">&nbsp; Alt. Telefone: <input type=text size=11 name=alt_phone maxlength=10 class="cust_form" value=""></td>
</tr><tr>
...
<span id="hotkeysdisplay"><a href="#" onMouseOver="HotKeys('ON')"><IMG SRC="../agc/images/vdc_XB_hotkeysactive_OFF_pt.gif" border=0 alt="CHAVES QUENTES INATIVAS"></a></span>
<? } ?>
</td></tr>
