...
#
#
# Copyright (C) 2008  Vicidial Group info@vicidial.com    LICENSE: AGPLv2
#
# CHANGELOG:
...
# 80506-1215 - mikec - Added support for multiserver logging
...
sub process_request {
    ...
    if ( !$VARDB_port ) { $VARDB_port = '3306'; }
    if ( !$AGILOGfile ) {
        $AGILOGfile = "$PATHlogs/FASTagiout.$year-$mon-$mday";
    }
    ...
    ### Grab Server values from the database
    $stmtA =
      "SELECT agi_output FROM servers where server_ip = '$VARserver_ip';";
...
"+++++++++++++++++ FastAGI Start ++++++++++++++++++++++++++++++++++++++++";
    ...
    if ( length( $ARGV[0] ) > 1 ) {
        ...
        if ($AGILOG) {
            $agi_string = "Asterisk Server's IP address = $ast_server_ip";
            &agi_output;
        }
    }
    $HVcauses = 0;
    ...
    while (<STDIN>) {
        ...
        if ( ( $request =~ /--HVcauses--/i ) && ( !$HVcauses ) ) {
            ...
            $agi_string =
"URL HVcauses: |$PRI|$DEBUG|$hangup_cause|$dialstatus|$dial_time|$ring_time|";
            &agi_output;
        }
    ...
    if ( $process =~ /^call_log/ ) {
        ...
        if ( $stage =~ /START/ ) {
            ...
            if ( $channel =~ /^Zap\// ) {
                ...
                $stmtA =
"SELECT count(*) FROM phones where server_ip='$ast_server_ip' and extension='$channel_line' and protocol='Zap';";
            ...
            if (   ( $callerid =~ /^V|^M/ )
                && ( $callerid =~ /\d\d\d\d\d\d\d\d\d/ )
                && ( length($number_dialed) < 1 ) )
            {
                $stmtA =
"SELECT cmd_line_b,cmd_line_d FROM vicidial_manager where callerid='$callerid' limit 1;";
                ...
                if ( length($number_dialed) < 1 ) {
                    $number_dialed = $extension;
                }
            }
            $stmtA =
"INSERT INTO call_log (uniqueid,channel,channel_group,type,server_ip,extension,number_dialed,start_time,start_epoch,end_time,end_epoch,length_in_sec,length_in_min,caller_code) values('$unique_id','$channel','$channel_group','$type','$ast_server_ip','$extension','$number_dialed','$now_date','$now_date_epoch','','','','','$callerid')";
...
"UPDATE call_log set end_time='$now_date',end_epoch='$now_date_epoch',length_in_sec=$length_in_sec,length_in_min='$length_in_min',channel='$channel' where uniqueid='$unique_id'";
            ...
            $stmtA =
"DELETE from live_inbound where uniqueid='$unique_id' and server_ip='$ast_server_ip'";
            if ($AGILOG) { $agi_string = "|$stmtA|"; &agi_output; }
            $affected_rows = $dbhA->do($stmtA);

            ##### BEGIN Park Log entry check and update #####
            $stmtA =
"SELECT UNIX_TIMESTAMP(parked_time),UNIX_TIMESTAMP(grab_time) FROM park_log where uniqueid='$unique_id' and server_ip='$ast_server_ip' LIMIT 1;";
            $sthA = $dbhA->prepare($stmtA) or die "preparing: ", $dbhA->errstr;
            $sthA->execute or die "executing: $stmtA ", $dbhA->errstr;
            $sthArows  = $sthA->rows;
            $rec_count = 0;
            ...
            if ($rec_count) {
                if ($AGILOG) {
                    $agi_string =
"*****Entry found for $unique_id-$ast_server_ip in park_log: $parked_time|$grab_time";
                    &agi_output;
                }
                ...
                $stmtA =
"UPDATE park_log set status='HUNGUP',hangup_time='$now_date',parked_sec='$parked_sec',talked_sec='$talked_sec' where uniqueid='$unique_id' and server_ip='$ast_server_ip'";
                $affected_rows = $dbhA->do($stmtA);
            }
...
"+++++ CALL LOG HUNGUP: |$unique_id|$channel|$extension|$now_date|min: $length_in_min|";
            ...
            if ( $DEBUG =~ /^DEBUG$/ ) {
                ...
                print out
"$now_date|$hangup_cause|$dialstatus|$dial_time|$ring_time|$unique_id|$channel|$extension|$type|$callerid|$calleridname|$priority|$ast_server_ip|\n";
            ...
            if ($AGILOG) {
                $agi_string =
                  "VD_hangup : $callerid $channel $priority $CIDlead_id";
                &agi_output;
            }
            ...
            if ( $channel =~ /^Local/ ) {
                if (
                       ( $PRI =~ /^PRI$/ )
                    && ( $callerid =~ /\d\d\d\d\d\d\d\d\d\d\d\d\d\d\d\d\d\d\d/ )
                    && (
                        ( $dialstatus =~ /BUSY/ )
...
"UPDATE vicidial_list set status='$VDL_status' where lead_id = '$CIDlead_id';";
                    ...
                    if ($AGILOG) {
                        $agi_string =
"--    VDAD vicidial_list update: |$VDADaffected_rows|$CIDlead_id";
                        &agi_output;
                    }
...
"UPDATE vicidial_auto_calls set status='$VDAC_status' where callerid = '$callerid';";
                    ...
                    if ($AGILOG) {
                        $agi_string =
                          "--    VDAC update: |$VDACaffected_rows|$CIDlead_id";
                        &agi_output;
                    }
...
"UPDATE vicidial_log FORCE INDEX(lead_id) set status='$VDL_status' where lead_id = '$CIDlead_id' and uniqueid LIKE \"$Euniqueid%\";";
                    ...
                    if ($AGILOG) {
                        $agi_string =
"--    VDAD vicidial_log update: |$VDLaffected_rows|$uniqueid|$VDACuniqueid|";
                        &agi_output;
                    }
                    ...
                    if ($AGILOG) {
                        $agi_string =
"--    VD_hangup Local DEBUG: |$PRI|$callerid|$dialstatus|$hangup_cause|";
                        &agi_output;
                    }
                }
                ...
                if ($AGILOG) {
                    $agi_string =
                      "+++++ VDAD START LOCAL CHANNEL: EXITING- $priority";
                    &agi_output;
                }
                ...
                $VD_alt_dial = 'NONE';
                $stmtA =
"SELECT lead_id,callerid,campaign_id,alt_dial,stage,UNIX_TIMESTAMP(call_time),uniqueid FROM vicidial_auto_calls where uniqueid = '$uniqueid' or callerid = '$callerid' limit 1;";
                if ($AGILOG) { $agi_string = "|$stmtA|"; &agi_output; }
                $sthA = $dbhA->prepare($stmtA)
                  or die "preparing: ", $dbhA->errstr;
                $sthA->execute or die "executing: $stmtA ", $dbhA->errstr;
...
"VD hangup: no VDAC record found: $uniqueid $calleridname";
                    ...
                    $stmtA =
"DELETE FROM vicidial_auto_calls where uniqueid='$uniqueid' or callerid = '$callerid' order by call_time desc limit 1;";
                    $affected_rows = $dbhA->do($stmtA);
                    if ($AGILOG) {
                        $agi_string =
"--    VDAC record deleted: |$affected_rows|   |$VD_lead_id|$uniqueid|$VD_uniqueid|$VD_callerid|$ast_server_ip";
                        &agi_output;
                    }
...
"CONNECTED TO DATABASE:  $queuemetrics_server_ip|$queuemetrics_dbname\n";
                        ...
                        $stmtB =
"SELECT agent from queue_log where call_id='$VD_callerid' and verb='CONNECT';";
                        $sthB = $dbhB->prepare($stmtB)
                          or die "preparing: ", $dbhB->errstr;
                        $sthB->execute
                          or die "executing: $stmtB ", $dbhB->errstr;
                        $sthBrows  = $sthB->rows;
                        $rec_count = 0;
                        while ( $sthBrows > $rec_count ) {
                            @aryB     = $sthB->fetchrow_array;
                        ...
                        if ( $rec_count < 1 ) {
                            $stmtB =
"INSERT INTO queue_log SET partition='P01',time_id='$secX',call_id='$VD_callerid',queue='$VD_campaign_id',agent='$VD_agent',verb='ABANDON',data1='1',data2='1',data3='$VD_stage',serverid='$queuemetrics_log_id';";
                            $Baffected_rows = $dbhB->do($stmtB);
                        }
                        else {
                            $stmtB =
"INSERT INTO queue_log SET partition='P01',time_id='$secX',call_id='$VD_callerid',queue='$VD_campaign_id',agent='$VD_agent',verb='COMPLETECALLER',data1='$VD_stage',data2='$VD_call_length',data3='1',serverid='$queuemetrics_log_id';";
                            $Baffected_rows = $dbhB->do($stmtB);
                        }
                    ...
                    $Euniqueid =~ s/\.\d+$//gi;
                    $stmtA =
"SELECT start_epoch,status,user,term_reason FROM vicidial_log FORCE INDEX(lead_id) where lead_id = '$VD_lead_id' and uniqueid LIKE \"$Euniqueid%\" limit 1;";
                    if ($AGILOG) { $agi_string = "|$stmtA|"; &agi_output; }
                    $sthA = $dbhA->prepare($stmtA)
                      or die "preparing: ", $dbhA->errstr;
                    $sthA->execute or die "executing: $stmtA ", $dbhA->errstr;
                    ...
                    if ( !$epc_countCUSTDATA ) {
                        if ($AGILOG) {
                            $agi_string =
"no VDL record found: $uniqueid $calleridname $VD_lead_id $uniqueid $VD_uniqueid";
                            &agi_output;
                        }

                        $secX    = time();
                        $Rtarget = ( $secX - 21600 )
                          ;    # look for VDCL entry within last 6 hours
                        (
                            $Rsec,  $Rmin,  $Rhour, $Rmday, $Rmon,
                            $Ryear, $Rwday, $Ryday, $Risdst
                        ) = localtime($Rtarget);
                        $Ryear = ( $Ryear + 1900 );
                        $Rmon++;
                        ...
                        $stmtA =
"SELECT start_epoch,status,closecallid,user,term_reason FROM vicidial_closer_log where lead_id = '$VD_lead_id' and call_date > \"$RSQLdate\" order by call_date desc limit 1;";
                        if ($AGILOG) { $agi_string = "|$stmtA|"; &agi_output; }
                        $sthA = $dbhA->prepare($stmtA)
                          or die "preparing: ", $dbhA->errstr;
                        $sthA->execute
                    ...
                    if ( !$epc_countCUSTDATA ) {
                        if ($AGILOG) {
                            $agi_string =
"no VDL or VDCL record found: $uniqueid $calleridname $VD_lead_id $uniqueid $VD_uniqueid";
                            &agi_output;
                        }
                    }
                    else {
                        ...
                        if ( $VD_status =~ /^NA$|^NEW$|^QUEUE$|^XFER$/ ) {
                            ...
                            else { $term_reason = 'CALLER'; }
                            $SQL_status =
                              "status='DROP',term_reason='$term_reason',";
...
"UPDATE vicidial_list set status='DROP' where lead_id = '$VD_lead_id';";
                            ...
                            if ($AGILOG) {
                                $agi_string =
"--    VDAD vicidial_list update: |$affected_rows|$VD_lead_id";
                                &agi_output;
                            }
                        }
                        else {
...
"UPDATE vicidial_log FORCE INDEX(lead_id) set $SQL_status end_epoch='$now_date_epoch',length_in_sec='$VD_seconds' where lead_id = '$VD_lead_id' and uniqueid LIKE \"$Euniqueid%\";";
                        ...
                        if ($AGILOG) {
                            $agi_string =
"--    VDAD vicidial_log update: |$VLaffected_rows|$uniqueid|$VD_status|";
                            &agi_output;
                        }
                        ...
                        if ( length($VD_closecallid) < 1 ) {
                            if ($AGILOG) {
                                $agi_string =
"no VDCL record found: $uniqueid $calleridname $VD_lead_id $uniqueid $VD_uniqueid";
                                &agi_output;
                            }
                        }
                        else {
                            if ( $VD_status =~ /^DONE$|^INCALL$|^XFER$/ ) {
                                ...
                                $VDCLSQL_status =
"status='DROP',queue_seconds='$VD_seconds',term_reason='$term_reason',";
                            }

                            $VD_seconds = ( $now_date_epoch - $VD_start_epoch );
                            $stmtA =
"UPDATE vicidial_closer_log set $VDCLSQL_status end_epoch='$now_date_epoch',length_in_sec='$VD_seconds' where closecallid = '$VD_closecallid';";
                            if ($AGILOG) {
                                $agi_string =
                                  "|$VDCLSQL_status|$VD_status|\n|$stmtA|";
                                &agi_output;
                            }
                            $affected_rows = $dbhA->do($stmtA);
                            if ($AGILOG) {
                                $agi_string =
"--    VDCL update: |$affected_rows|$uniqueid|$VD_closecallid|";
                                &agi_output;
                            }
                    ...
                    $stmtA =
"SELECT auto_alt_dial,auto_alt_dial_statuses FROM vicidial_campaigns where campaign_id='$VD_campaign_id';";
                    if ($AGILOG) { $agi_string = "|$stmtA|"; &agi_output; }
                    $sthA = $dbhA->prepare($stmtA)
                      or die "preparing: ", $dbhA->errstr;
                    $sthA->execute or die "executing: $stmtA ", $dbhA->errstr;
...
"SELECT alt_phone,gmt_offset_now,state,list_id FROM vicidial_list where lead_id='$VD_lead_id';";
                            ...
                            if ( length($VD_alt_phone) > 5 ) {
                                $stmtA =
"INSERT INTO vicidial_hopper SET lead_id='$VD_lead_id',campaign_id='$VD_campaign_id',status='READY',list_id='$VD_list_id',gmt_offset_now='$VD_gmt_offset_now',state='$VD_state',alt_dial='ALT',user='',priority='25';";
                                $affected_rows = $dbhA->do($stmtA);
                                if ($AGILOG) {
                                    $agi_string =
"--    VDH record inserted: |$affected_rows|   |$stmtA|";
                                    &agi_output;
                                }
                            }
                        }
                        if (
                            (
                                   ( $VD_auto_alt_dial =~ /(ADDR3_ONLY)/ )
                                && ( $VD_alt_dial =~ /NONE|MAIN/ )
                            )
                            || (   ( $VD_auto_alt_dial =~ /(ALT_AND_ADDR3)/ )
                                && ( $VD_alt_dial =~ /ALT/ ) )
                          )
                        {
                            $VD_address3 = '';
...
"SELECT address3,gmt_offset_now,state,list_id FROM vicidial_list where lead_id='$VD_lead_id';";
                            ...
                            if ( length($VD_address3) > 5 ) {
                                $stmtA =
"INSERT INTO vicidial_hopper SET lead_id='$VD_lead_id',campaign_id='$VD_campaign_id',status='READY',list_id='$VD_list_id',gmt_offset_now='$VD_gmt_offset_now',state='$VD_state',alt_dial='ADDR3',user='',priority='20';";
                                $affected_rows = $dbhA->do($stmtA);
                                if ($AGILOG) {
                                    $agi_string =
"--    VDH record inserted: |$affected_rows|   |$stmtA|";
